...
| GET    | `/catalogue/audio/genres`, `/catalogue/audio/voices`, `/catalogue/audio/dub-languages` | catalogue:read    | The catalogues the studio's pickers use                                                                                                         |
...
| `COLLAGE`            | 2                                                      | `sourceKeys[]` (2–9, in order), `layout: auto \| grid \| hero \| row \| stack \| before_after`, `aspect`, `gap`, `background`, `rounded`, `labels[]`, `sizes[]`, `price`, `businessName`. Ours end to end — no vendor |
...
| `VOICEOVER`          | 8                                                      | `script`, `voiceId` (from `/catalogue/audio/voices`), `style`, `speed`                                                                                                                                                |
| `MUSIC`              | 10 preview + 30 unlock                                 | `brief`, `genre` (from `/catalogue/audio/genres`), `vocal`, `language`, `durationSec`, optional `lyrics`. Returns a 30-second `preview` and a locked `audio`; `/unlock` opens it                                      |
| `DUB`                | 90 · 240 with lips                                     | `sourceKey` (video), `targetLanguage` (from `/catalogue/audio/dub-languages`), `lipsync`, `speakers`, `keepBackground`, `quality`, **`consent: true`**                                                                |
