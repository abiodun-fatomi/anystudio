...
import { IsBoolean, IsEmail, IsIn, IsNotEmpty, IsOptional, IsString, IsUUID, Length, Matches, MaxLength, MinLength, ValidateNested } from 'class-validator';
...
export class ChangeEmailDto extends ReauthDto {
  @ApiProperty({ example: 'ada@newshop.ng', maxLength: 320 })
  @IsEmail()
  @MaxLength(320)
  email: string;
}
...
export class MfaCodeDto {
  @ApiProperty({ example: '123456', description: 'Six digits from the authenticator app' })
  @IsString()
  @Length(6, 12)
  code: string;
}
...
export class NotificationSwitchesDto {
  @ApiPropertyOptional({ description: 'Email when a video or batch finishes while you are away' })
  @IsOptional()
  @IsBoolean()
  generationDoneEmail?: boolean;

  @ApiPropertyOptional({ description: 'WhatsApp message when a video or batch finishes' })
  @IsOptional()
  @IsBoolean()
  generationDoneWhatsApp?: boolean;

  @ApiPropertyOptional({ description: 'Email when credits run low' })
  @IsOptional()
  @IsBoolean()
  lowCreditsEmail?: boolean;

  @ApiPropertyOptional({ description: 'A weekly note on what was made and what worked' })
  @IsOptional()
  @IsBoolean()
...
export class NotificationsDto {
  @ApiPropertyOptional({ type: NotificationSwitchesDto })
  @IsOptional()
  @ValidateNested()
  @Type(() => NotificationSwitchesDto)
  ...
  @ApiPropertyOptional({ description: 'Where the choice was made, for the consent record' })
  @IsOptional()
  @IsString()
  @MaxLength(500)
  sourceUrl?: string;
...
export class DeleteAccountDto extends ReauthDto {
  @ApiProperty({ description: 'Must be the literal word DELETE, typed by the person' })
  @IsIn(['DELETE'])
  confirm: 'DELETE';
}
...
export const NOTIFICATION_DEFAULTS: NotificationSwitches = {
  generationDoneEmail: true,
  generationDoneWhatsApp: true,
