...
export class SupportController {
  ...
  current(@CurrentActor() a: Actor) { return this.support.current(a); }

  @Get('/history')
  @ApiOperation({ summary: 'Closed chats, newest first' })
  history(@CurrentActor() a: Actor) { return this.support.history(a); }

  @Post()
  @ApiOperation({ summary: 'Open a chat (or return the one already open)' })
  open(@CurrentActor() a: Actor, @Body() b: OpenConversationDto, @Req() req: Request) { return this.support.open(a, b, req); }

  @Get('/:id')
  one(@CurrentActor() a: Actor, @Param('id', ParseUUIDPipe) id: string) { return this.support.one(a, id); }

  @Post('/:id/messages') @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Say something; the assistant answers in the same response' })
  send(@CurrentActor() a: Actor, @Param('id', ParseUUIDPipe) id: string, @Body() b: SupportMessageDto, @Req() req: Request) { return this.support.send(a, id, b, req); }

  @Post('/:id/close') @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'End the chat; a transcript is emailed' })
  close(@CurrentActor() a: Actor, @Param('id', ParseUUIDPipe) id: string, @Body() b: CloseConversationDto) { return this.support.close(a, id, b); }
}
