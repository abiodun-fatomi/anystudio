...
export interface MessageView { id: string; role: SupportRole; text: string; who: string | null; createdAt: string }
export interface ConversationView {
  id: string; status: 'OPEN' | 'CLOSED'; topic: string | null; needsHuman: boolean; staffJoined: boolean;
  createdAt: string; closedAt: string | null; transcriptSentAt: string | null; messages: MessageView[];
}
...
export class SupportService {
  ...
  async open(actor: Actor, dto: OpenConversationDto, req: Request) {
    const existing = await this.db.supportConversation.findFirst({ where: { userId: actor.userId, status: 'OPEN' }, include: { messages: { orderBy: { createdAt: 'asc' } } } });
    ...
    const row = await this.db.supportConversation.create({
      data: {
        userId: actor.userId, workspaceId, page: dto.page?.slice(0, 300) ?? null, messageCount: 1,
  ...
  async current(actor: Actor) {
    const row = await this.db.supportConversation.findFirst({ where: { userId: actor.userId, status: 'OPEN' }, include: { messages: { orderBy: { createdAt: 'asc' } } } });
  ...
  async history(actor: Actor) {
    ...
    return Helpers.successResponse(200, 'OK', rows.map((r) => ({ id: r.id, topic: r.topic, createdAt: r.createdAt.toISOString(), closedAt: r.closedAt?.toISOString() ?? null, messageCount: r.messageCount })));
  }
  ...
  async send(actor: Actor, id: string, dto: SupportMessageDto, req: Request) {
    ...
    await this.db.supportConversation.update({ where: { id }, data: { messageCount: { increment: 1 }, lastMessageAt: new Date(), ...(dto.page ? { page: dto.page.slice(0, 300) } : {}) } });
    ...
    const [user, workspace] = await Promise.all([
      ...
      convo.workspaceId ? this.db.workspace.findUnique({ where: { id: convo.workspaceId }, select: { name: true, type: true, wallet: { select: { id: true } } } }) : null,
    ]);
    ...
    const turns: AssistantTurn[] = [...convo.messages, mine]
      ...
      .map((m) => ({ role: m.role === 'USER' ? 'user' : 'assistant', text: m.role === 'STAFF' ? `[A member of the AnyStudio team wrote:] ${m.text}` : m.text }));

    const answer = await this.assistant.answer(turns, { userName: user.name, workspace: workspace ? { name: workspace.name, type: workspace.type, balance } : null, page: dto.page ?? convo.page }, id);
    const reply = await this.db.supportMessage.create({ data: { conversationId: id, role: 'ASSISTANT', text: answer.reply, meta: answer.meta as Prisma.InputJsonValue } });
    const becameHuman = answer.needsHuman && !convo.needsHuman;
    await this.db.supportConversation.update({
      where: { id },
      data: { messageCount: { increment: 1 }, lastMessageAt: new Date(), ...(answer.needsHuman ? { needsHuman: true } : {}), ...(convo.topic ? {} : { topic: answer.topic }) },
    });
    logger.info({ conversationId: id, userId: actor.userId, chars: text.length, needsHuman: answer.needsHuman, escalated: becameHuman, fallback: answer.meta.fallback ?? null, ip: req.ip }, 'support.message');
  ...
  async list(q: SupportListQueryDto) {
    ...
    else if (q.filter === 'needs_human') { where.status = 'OPEN'; where.needsHuman = true; }
    else if (q.filter === 'closed') where.status = 'CLOSED';
    if (q.q) where.OR = [{ topic: { contains: q.q, mode: 'insensitive' } }, { user: { email: { contains: q.q, mode: 'insensitive' } } }, { user: { name: { contains: q.q, mode: 'insensitive' } } }];
    const rows = await this.db.supportConversation.findMany({
      where, orderBy: [{ needsHuman: 'desc' }, { lastMessageAt: 'desc' }], take: take + 1,
      ...(q.cursor ? { cursor: { id: q.cursor }, skip: 1 } : {}),
      include: { user: { select: { id: true, name: true, email: true } }, messages: { orderBy: { createdAt: 'desc' }, take: 1, where: { role: { not: 'SYSTEM' } } } },
    });
    ...
    return Helpers.successResponse(200, 'OK', {
      ...
      rows: page.map((r) => ({
        id: r.id, status: r.status, topic: r.topic, needsHuman: r.needsHuman, staffJoined: Boolean(r.staffJoinedAt), page: r.page,
        user: r.user, workspaceId: r.workspaceId, messageCount: r.messageCount, lastMessageAt: r.lastMessageAt.toISOString(), createdAt: r.createdAt.toISOString(),
  ...
  async staffOne(id: string) {
    const row = await this.db.supportConversation.findUnique({
      where: { id },
      include: { messages: { orderBy: { createdAt: 'asc' } }, user: { select: { id: true, name: true, email: true, phone: true, status: true, createdAt: true } } },
    });
    if (!row) throw new NotFoundError('Conversation');
    const workspace = row.workspaceId ? await this.db.workspace.findUnique({ where: { id: row.workspaceId }, select: { id: true, name: true, type: true } }) : null;
    return Helpers.successResponse(200, 'OK', { ...this.view(row), page: row.page, user: row.user, workspace, messagesMeta: row.messages.map((m) => ({ id: m.id, meta: m.meta })) });
  }
  ...
  async staffReply(actor: Actor, id: string, dto: StaffReplyDto, req: Request) {
    ...
    const row = await this.db.supportMessage.create({ data: { conversationId: id, role: 'STAFF', text: dto.text.trim(), meta: { staffId: actor.userId, name: who } } });
    await this.db.supportConversation.update({ where: { id }, data: { messageCount: { increment: 1 }, lastMessageAt: new Date(), staffJoinedAt: convo.staffJoinedAt ?? new Date() } });
    await this.notifications.notify(convo.userId, { workspaceId: convo.workspaceId, kind: 'SYSTEM', title: `${who} from AnyStudio replied in your help chat`, body: dto.text.trim().slice(0, 140), href: `/today?support=${id}`, refId: `support:${id}` }).catch((err: unknown) => logger.warn({ err, conversationId: id }, 'support.staff_reply: bell notification failed'));
  ...
  async staffClose(actor: Actor, id: string, req: Request) {
    ...
    const convo = await this.db.supportConversation.findUnique({ where: { id } });
    if (!convo) throw new NotFoundError('Conversation');
    const closed = convo.status === 'OPEN' ? await this.finish(id, `staff:${actor.userId}`, true) : await this.db.supportConversation.findUniqueOrThrow({ where: { id }, include: { messages: { orderBy: { createdAt: 'asc' } } } });
  ...
  async staffResolve(actor: Actor, id: string, req: Request) {
    assertStaffMutation(actor, { min: 'SUPPORT', stepUpMinutes: STEP_UP_MIN });
    const row = await this.db.supportConversation.update({ where: { id }, data: { needsHuman: false }, include: { messages: { orderBy: { createdAt: 'asc' } } } });
  ...
  async sweepIdle(): Promise<number> {
    const stale = await this.db.supportConversation.findMany({ where: { status: 'OPEN', lastMessageAt: { lt: new Date(Date.now() - IDLE_CLOSE_MS) } }, select: { id: true }, take: 200 });
    for (const s of stale) await this.finish(s.id, 'idle', true).catch((err: unknown) => logger.error({ err, conversationId: s.id }, 'support.sweep: close failed'));
  ...
  private async finish(id: string, by: string, email: boolean): Promise<SupportConversation & { messages: SupportMessage[] }> {
    ...
    const row = await this.db.supportConversation.update({
      where: { id }, data: { status: 'CLOSED', closedAt: now, closedBy: by },
    ...
    if (!email) sent = 'skipped';
      ...
      try {
        await this.mailer.send(supportTranscript(row.user.email, row.user.name, {
          topic: row.topic, openedAt: row.createdAt, closedAt: now, needsHuman: row.needsHuman,
          lines: row.messages.map((m) => ({ role: m.role, text: m.text, at: m.createdAt, who: m.role === 'STAFF' ? ((m.meta as { name?: string } | null)?.name ?? null) : null })),
        }));
  ...
  private view(row: SupportConversation & { messages: SupportMessage[] }): ConversationView {
    return {
      id: row.id, status: row.status, topic: row.topic, needsHuman: row.needsHuman, staffJoined: Boolean(row.staffJoinedAt),
      createdAt: row.createdAt.toISOString(), closedAt: row.closedAt?.toISOString() ?? null, transcriptSentAt: row.transcriptSentAt?.toISOString() ?? null,
      messages: row.messages.map((m) => this.msg(m)),
    };
  }

  private msg(m: SupportMessage): MessageView {
    return { id: m.id, role: m.role, text: m.text, who: m.role === 'STAFF' ? ((m.meta as { name?: string } | null)?.name ?? 'AnyStudio') : null, createdAt: m.createdAt.toISOString() };
  }
}
