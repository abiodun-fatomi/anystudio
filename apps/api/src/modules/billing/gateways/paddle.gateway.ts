...
interface PaddleTxn {
  id: string; status: string; customer_id?: string | null; subscription_id?: string | null; currency_code: string; origin?: string;
  ...
  billed_at?: string | null; created_at?: string;
}

interface PaddleSub {
  id: string; status: string; customer_id?: string; custom_data?: Record<string, unknown> | null;
...
export class PaddleGateway implements Gateway {
  ...
  constructor(private readonly apiKey: string, private readonly webhookSecret: string, env: 'sandbox' | 'live') {
  ...
  async createCheckout(req: CheckoutRequest): Promise<CheckoutSession> {
    if (!req.item.providerRef) throw new Error(`${req.item.kind} ${req.item.code} has no Paddle price id${req.item.interval ? ` for ${req.item.interval}` : ''}`);
    const body = {
  ...
  async verify(payment: Payment, hint?: { providerRef?: string }): Promise<Verification> {
    ...
    const ours = t.custom_data?.paymentId === payment.id || t.custom_data?.reference === payment.reference
      || (payment.kind === 'RENEWAL' && t.subscription_id && t.subscription_id === (payment.providerPayload as { subscriptionRef?: string } | null)?.subscriptionRef);
    ...
    return {
      ...
      raw: { id: t.id, status: t.status, priceId, total, currency: t.currency_code, origin: t.origin, subscription_id: t.subscription_id, customer_id: t.customer_id, billed_at: t.billed_at },
    };
  }
  ...
  static verifySignature(header: string, rawBody: Buffer, secret: string, now = Date.now()): boolean {
    ...
    const ts = parts.ts;
    const h1 = header.split(';').filter((p) => p.trim().startsWith('h1=')).map((p) => p.trim().slice(3));
  ...
  interpret(parsed: ParsedWebhook): WebhookIntent {
    const b = (parsed.body ?? {}) as { event_type?: string; data?: PaddleTxn & PaddleSub & { transaction_id?: string; action?: string; totals?: { total?: string } } };
    ...
    switch (parsed.type) {
      ...
      case 'transaction.paid':
        return { kind: 'charge', reference, providerRef: d.id, customerRef: d.customer_id ?? undefined, subscriptionRef: d.subscription_id ?? undefined, status: 'succeeded' };
      case 'transaction.payment_failed':
      case 'transaction.canceled':
        return { kind: 'charge', reference, providerRef: d.id, customerRef: d.customer_id ?? undefined, subscriptionRef: d.subscription_id ?? undefined, status: 'failed' };
      ...
      case 'subscription.canceled': {
        const status = d.status === 'active' || d.status === 'trialing' ? 'active' : d.status === 'past_due' ? 'past_due' : d.status === 'paused' ? 'paused' : d.status === 'canceled' ? 'cancelled' : 'active';
        return {
          kind: 'subscription', subscriptionRef: d.id, customerRef: d.customer_id, reference, status,
      ...
      case 'adjustment.updated': {
        ...
        return a.transaction_id ? { kind: 'refund', providerRef: a.transaction_id, amountMinor: a.totals?.total ? Number(a.totals.total) : undefined } : { kind: 'ignore', why: 'refund without transaction id' };
      }
      default:
  ...
  async cancelSubscription(subscriptionRef: string, atPeriodEnd: boolean): Promise<void> {
    await http('paddle', `${this.base}/subscriptions/${encodeURIComponent(subscriptionRef)}/cancel`, {
      body: { effective_from: atPeriodEnd ? 'next_billing_period' : 'immediately' }, headers: this.headers(), timeoutMs: TIMEOUT,
    });
  }
}
