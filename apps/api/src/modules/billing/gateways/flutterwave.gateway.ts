...
export class FlutterwaveGateway implements Gateway {
  ...
  async verify(payment: Payment, hint?: { providerRef?: string }): Promise<Verification> {
    ...
    if (tx.tx_ref !== payment.reference && payment.kind !== 'RENEWAL')
      return { ok: false, state: 'failed', reason: `reference mismatch: ${tx.tx_ref}`, raw: tx };
    ...
    switch (parsed.type) {
      case 'charge.completed': {
        ...
        return {
          ...
          customerRef: d.customer?.email?.toLowerCase(),
          subscriptionRef: planRef,
          planRef,
          customerEmail: d.customer?.email?.toLowerCase(),
          status,
        };
      }
      case 'subscription.cancelled': {
        ...
        return {
          kind: 'subscription',
          subscriptionRef: String(sub.plan ?? sub.id ?? ''),
          customerRef: sub.customer?.email?.toLowerCase(),
          status: 'cancelled',
        };
      }
      ...
      case 'charge.refunded': {
        ...
        return ref !== undefined
          ? {
              ...
              amountMinor: r.amount_refunded !== undefined && r.currency ? toMinor(r.amount_refunded, r.currency) : undefined,
            }
