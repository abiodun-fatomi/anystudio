...
export class CreateApiKeyDto {
  ...
  @ApiProperty({ example: 'Production server' }) @IsString() @MinLength(2) @MaxLength(80)
  name!: string;

  @ApiPropertyOptional({ enum: API_SCOPES, isArray: true, description: 'Defaults to every scope' })
  @IsOptional() @IsArray() @ArrayMaxSize(10) @IsIn(API_SCOPES as readonly string[], { each: true })
  scopes?: string[];
...
export class CreateWebhookDto {
  ...
  @IsUrl({ protocols: ['https'], require_protocol: true, require_tld: process.env.APP_ENV === 'production' }) @MaxLength(500)
  url!: string;
  ...
  @IsOptional() @IsArray() @IsIn(WEBHOOK_EVENTS as readonly string[], { each: true })
  events?: string[];
}

export class UpdateWebhookDto {
  @ApiPropertyOptional() @IsOptional() @IsUrl({ protocols: ['https'], require_protocol: true, require_tld: process.env.APP_ENV === 'production' }) @MaxLength(500)
  url?: string;

  @ApiPropertyOptional({ enum: WEBHOOK_EVENTS, isArray: true }) @IsOptional() @IsArray() @IsIn(WEBHOOK_EVENTS as readonly string[], { each: true })
  events?: string[];

  @ApiPropertyOptional({ description: 'Pause (false) or resume (true); resuming clears the failure count' }) @IsOptional() @IsBoolean()
  active?: boolean;
}
...
export class MerchantRefDto {
  @ApiPropertyOptional({ description: 'Your reference for the merchant this is for (a store id, a customer id). Opaque to us; used for fair-use limits and usage rollups.' })
