...
const CAPABILITY_WORD: Record<string, string> = {
  IMAGE_EDIT: 'product photo', IMAGE_GENERATE: 'flyer', BACKGROUND_REMOVE: 'cut-out', BACKGROUND_REPLACE: 'new background', RELIGHT: 'relit photo', UPSCALE: 'enhanced photo',
  IMAGE_TO_VIDEO: 'video', VIDEO_STITCH: 'ad', TEXT_GENERATE: 'listing copy', VOICEOVER: 'voiceover', MUSIC: 'song', DUB: 'translated video', LIPSYNC: 'lip-synced video',
};

@Injectable()
export class NotificationService implements OnModuleInit {
  constructor(private readonly db: PrismaClient, private readonly hooks: GenerationHooks) {}
  ...
  async onGenerationFinished(row: Generation): Promise<void> {
    ...
    if (row.status === 'SUCCEEDED') {
      await this.notify(row.requestedById, { workspaceId: row.workspaceId, kind: 'GENERATION_DONE', title: `${title} is ready`, body: `Your ${word} finished. Open it in the studio or find it in the library.`, href: `/library?open=${row.id}`, refId: row.id });
    } else if (row.status === 'FAILED') {
      await this.notify(row.requestedById, { workspaceId: row.workspaceId, kind: 'GENERATION_FAILED', title: `${title} did not work`, body: `${customerMessage(row) ?? 'Something went wrong.'} ${row.credits ? `${row.credits} credits are back.` : ''}`.trim(), href: '/studio', refId: row.id });
    }
  }

  /** Write one. Never throws into the caller. */
  async notify(userId: string, n: { workspaceId?: string | null; kind: NotificationKind; title: string; body?: string | null; href?: string | null; refId?: string | null }): Promise<void> {
    try {
      ...
      await this.db.notification.create({ data: { userId, workspaceId: n.workspaceId ?? null, kind: n.kind, title: n.title.slice(0, 160), body: n.body?.slice(0, 600) ?? null, href: n.href ?? null, refId: n.refId ?? null } });
    } catch (err) {
  ...
  async notifyWorkspace(workspaceId: string, exceptUserId: string | null, n: { kind: NotificationKind; title: string; body?: string | null; href?: string | null; refId?: string | null }): Promise<void> {
    const members = await this.db.workspaceMember.findMany({ where: { workspaceId, ...(exceptUserId ? { userId: { not: exceptUserId } } : {}) }, select: { userId: true } });
  ...
  async list(userId: string, opts: { take?: number; cursor?: string; unreadOnly?: boolean } = {}): Promise<{ items: NotificationItem[]; nextCursor: string | null; unread: number }> {
    ...
    const items: NotificationItem[] = [
      ...personal.map((n) => ({ id: n.id, kind: n.kind as NotificationItem['kind'], title: n.title, body: n.body, href: n.href, refId: n.refId, read: Boolean(n.readAt), createdAt: n.createdAt })),
  ...
  async markRead(userId: string, ids: string[] | 'all'): Promise<{ unread: number }> {
    if (ids === 'all') {
      ...
      for (const p of live.filter((x) => !x.read)) await this.db.platformMessageRead.upsert({ where: { messageId_userId: { messageId: p.id.slice(3), userId } }, create: { messageId: p.id.slice(3), userId }, update: {} });
    } else {
      ...
      for (const messageId of platform) await this.db.platformMessageRead.upsert({ where: { messageId_userId: { messageId, userId } }, create: { messageId, userId }, update: {} }).catch(() => undefined);
    }
  ...
  private async platformFor(userId: string, before: Date | null): Promise<NotificationItem[]> {
    ...
    const rows = await this.db.platformMessage.findMany({
      ...
      orderBy: { publishedAt: 'desc' }, take: 20, include: { reads: { where: { userId }, select: { readAt: true } } },
    });
    return rows
      .filter((m) => m.audience === 'ALL' || mine.has(m.audience as WorkspaceType))
      .map((m) => ({ id: `pm:${m.id}`, kind: 'PLATFORM' as const, title: m.title, body: m.body, href: m.href, refId: null, read: m.reads.length > 0, createdAt: m.publishedAt ?? m.createdAt }));
  }
  ...
  async createPlatformMessage(createdById: string, dto: { title: string; body: string; href?: string; audience?: 'ALL' | 'PERSONAL' | 'BUSINESS' | 'ORGANIZATION'; publish?: boolean; expiresAt?: string }) {
    return this.db.platformMessage.create({ data: { title: dto.title.trim(), body: dto.body.trim(), href: dto.href?.trim() || null, audience: dto.audience ?? 'ALL', publishedAt: dto.publish ? new Date() : null, expiresAt: dto.expiresAt ? new Date(dto.expiresAt) : null, createdById } });
  }

  async updatePlatformMessage(id: string, dto: { title?: string; body?: string; href?: string | null; audience?: 'ALL' | 'PERSONAL' | 'BUSINESS' | 'ORGANIZATION'; published?: boolean; expiresAt?: string | null }) {
