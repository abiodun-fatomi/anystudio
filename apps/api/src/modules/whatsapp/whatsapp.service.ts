...
export class WhatsappService implements OnModuleInit {
  ...
  verify(query: Record<string, unknown>): string | null {
    ...
    if (query['hub.mode'] === 'subscribe' && query['hub.verify_token'] === expected && typeof query['hub.challenge'] === 'string')
      ...
      for (const change of entry.changes ?? []) {
        ...
        for (const m of value.messages) {
          try {
            ...
            await this.client
              .send(m.from, { kind: 'text', text: 'Something went wrong on our side. Nothing was charged — try that again in a moment.' })
              .catch(() => undefined);
          }
        }
      }
  ...
  async handleInbound(waId: string, name: string | null, waMessageId: string, inbound: Inbound): Promise<'handled' | 'duplicate' | 'throttled' | 'opted_out'> {
    if (!this.allow(waId)) {
      logger.warn({ from: mask(waId) }, 'whatsapp: too many messages from one number; dropping');
      return 'throttled';
    }
    ...
    try {
      await this.db.whatsappMessage.create({
        data: { contactId: contact.id, waMessageId, direction: 'IN', type: inbound.kind, body: inbound as unknown as Prisma.InputJsonObject },
      });
    } catch (err) {
    ...
    if (STOP_WORDS.has(text)) {
      ...
      await this.say(contact, [
        { kind: 'text', text: 'Okay — we will not message you again unless you write to us. Send "hi" whenever you want to make something.' },
      ]);
      return 'opted_out';
    }
    if (inbound.kind === 'text' && MENU_WORDS.has(text)) {
      await this.menu(contact, true);
      return 'handled';
    }
    if (inbound.kind === 'choice' && inbound.id === 'menu') {
      await this.menu(contact);
      return 'handled';
    }
    ...
    if (inbound.kind === 'image') {
      await this.photoReceived(contact, inbound);
      return 'handled';
    }
    if (inbound.kind === 'choice' && inbound.id.startsWith('unlock:')) {
      await this.unlock(contact, inbound.id.slice(7));
      return 'handled';
    }
    if (inbound.kind === 'choice' && inbound.id.startsWith('buy:')) {
      await this.buy(contact, inbound.id.slice(4));
      return 'handled';
    }
    if (inbound.kind === 'choice' && inbound.id === 'credits') {
      await this.credits(contact);
      return 'handled';
    }

    switch (state.flow) {
      case 'photo':
        await this.photoFlow(contact, state, inbound);
        break;
      case 'song':
        await this.songFlow(contact, state, inbound);
        break;
      case 'voice':
        await this.voiceFlow(contact, state, inbound);
        break;
      case 'copy':
        await this.copyFlow(contact, state, inbound);
        break;
      default:
        await this.idle(contact, state, inbound);
    }
    return 'handled';
  }
  ...
  private async idle(contact: WhatsappContact, state: BotState, inbound: Inbound): Promise<void> {
    if (inbound.kind === 'choice') {
      switch (inbound.id) {
        case 'flow:photo':
          await this.say(contact, [{ kind: 'text', text: 'Send me a product photo — a plain phone photo is fine. Then I will ask what to do with it.' }]);
          return;
        case 'flow:song':
          await this.startSong(contact);
          return;
        case 'flow:voice':
          await this.setState(contact, { flow: 'voice', step: 'script', sourceKey: state.sourceKey });
          await this.say(contact, [{ kind: 'text', text: 'Type the script and I will record it. About 150 words is a minute.' }]);
          return;
        case 'flow:copy':
          await this.setState(contact, { flow: 'copy', step: 'details', sourceKey: state.sourceKey });
          await this.say(contact, [
            {
              kind: 'text',
              text: 'What is the product? Put the name on the first line, then the price and anything a buyer should know.\n\ne.g.\nAnkara tote bag\n₦12,000, handmade in Lagos, fits a laptop',
            },
          ]);
          return;
        default:
  ...
  private async photoReceived(contact: WhatsappContact, inbound: Extract<Inbound, { kind: 'image' }>): Promise<void> {
    ...
    if (inbound.caption && inbound.caption.length > 3) {
      await this.requestScene(contact, asset.key, inbound.caption);
      return;
    }
    await this.say(contact, [
      {
        kind: 'buttons',
        text: 'Got it. What shall I do with this one?',
        footer: 'Or just type where the product should be, e.g. "on a marble counter"',
        buttons: [
          { id: 'photo:scene', title: 'New scene · 10 cr' },
          { id: 'photo:cutout', title: 'Cut out · 2 cr' },
          { id: 'photo:reel', title: 'Reel · 120 cr' },
        ],
      },
    ]);
  }
  ...
  private async photoFlow(contact: WhatsappContact, state: BotState, inbound: Inbound): Promise<void> {
    ...
    if (inbound.kind === 'choice') {
      switch (inbound.id) {
        case 'photo:scene':
          await this.setState(contact, { ...state, step: 'scene' });
          await this.say(contact, [
            { kind: 'text', text: 'Where should the product be? One line is enough — "on a marble kitchen counter in soft morning light".' },
          ]);
          return;
        case 'photo:cutout':
          await this.request(contact, 'BACKGROUND_REMOVE', { sourceKey, background: 'transparent' }, 'Cutting it out…');
          return;
        case 'photo:reel':
          await this.setState(contact, { ...state, step: 'reel' });
          await this.say(contact, [
            {
              kind: 'buttons',
              text: 'A 5-second reel of this product costs 120 credits and takes a few minutes. Go ahead?',
              buttons: [
                { id: 'photo:reel:yes', title: 'Yes, make it' },
                { id: 'menu', title: 'No' },
              ],
            },
          ]);
        ...
        case 'photo:reel:yes':
          await this.request(
            ...
            {
              sourceKey,
              prompt: 'The camera slowly pushes in on the product as soft light sweeps across it',
              shots: 1,
              durationSec: 5,
              aspect: '9:16',
              format: 'reveal',
            ...
            'Rendering your reel — this takes a few minutes.',
          );
          return;
        default:
          break;
    ...
    if (inbound.kind === 'text' && inbound.text.length > 2) {
      await this.requestScene(contact, sourceKey, inbound.text);
      return;
    }
  ...
  private async requestScene(contact: WhatsappContact, sourceKey: string, prompt: string): Promise<void> {
    await this.request(
      contact,
      'IMAGE_EDIT',
      { sourceKey, prompt: prompt.slice(0, 600), preserveProduct: true, aspect: '1:1', sizes: ['feed_square', 'story'] },
      'Placing your product in the scene — about 20 seconds.',
    );
  }
    ...
    await this.say(contact, [
      {
        ...
        header: 'A song',
        button: 'Pick a genre',
        text: 'Which sound? Pick one, or choose "Another" and type it — we have 68 genres from Afrobeats to cumbia.',
        sections: [
          {
            rows: [
              ...genres.map((g) => ({ id: `genre:${g.key}`, title: g.name.slice(0, 24), description: g.region.slice(0, 72) })),
              { id: 'genre:other', title: 'Another genre', description: 'Type its name' },
            ],
          },
        ],
      },
  ...
  private async songFlow(contact: WhatsappContact, state: BotState, inbound: Inbound): Promise<void> {
    ...
    if (state.step === 'genre') {
      ...
      if (inbound.kind === 'choice' && inbound.id.startsWith('genre:') && inbound.id !== 'genre:other') key = inbound.id.slice(6);
        ...
        if (inbound.kind === 'choice') {
          await this.say(contact, [{ kind: 'text', text: 'Type the genre — "highlife", "gospel", "k-pop", "cumbia"…' }]);
          return;
        }
        const q = inbound.text.toLowerCase();
        const found = await this.db.musicGenre.findFirst({
          where: { active: true, OR: [{ key: q.replace(/\s+/g, '-') }, { name: { contains: q, mode: 'insensitive' } }] },
          orderBy: { sort: 'asc' },
        });
        if (!found) {
          await this.say(contact, [{ kind: 'text', text: `I do not have "${inbound.text}" yet. Try another name, or say "menu".` }]);
          return;
        }
        key = found.key;
      }
      if (!key) {
        await this.say(contact, [{ kind: 'text', text: 'Pick a genre from the list, or type one.' }]);
        return;
      }
    ...
    if (state.step === 'brief') {
      if (inbound.kind !== 'text' || inbound.text.length < 3) {
        await this.say(contact, [{ kind: 'text', text: 'Tell me what the song is about, in a sentence or two.' }]);
        return;
      }
      ...
      await this.say(contact, [
        {
          ...
          text: 'Who sings it?',
          buttons: [
            { id: 'vocal:female', title: 'Female voice' },
            { id: 'vocal:male', title: 'Male voice' },
            { id: 'vocal:instrumental', title: 'No vocals' },
          ],
        },
      ]);
      return;
    ...
    if (state.step === 'vocal') {
      const vocal =
        inbound.kind === 'choice' && inbound.id.startsWith('vocal:')
          ? inbound.id.slice(6)
          : inbound.kind === 'text'
            ? /male|man|guy/i.test(inbound.text) && !/female/i.test(inbound.text)
              ? 'male'
              : /instrument|no vocal|beat/i.test(inbound.text)
                ? 'instrumental'
                : 'female'
            : null;
      if (!vocal || !data.genre || !data.brief) {
        await this.say(contact, [{ kind: 'text', text: 'Tap Female, Male or No vocals.' }]);
        return;
      }
      await this.request(
        contact,
        'MUSIC',
        { brief: data.brief, genre: data.genre, vocal, language: 'en', durationSec: 120 },
        'Composing — a full song takes a minute or two. You will hear a 30-second preview first.',
      );
      return;
    }
    await this.startSong(contact);
  ...
  private async voiceFlow(contact: WhatsappContact, _state: BotState, inbound: Inbound): Promise<void> {
    if (inbound.kind !== 'text' || inbound.text.length < 3) {
      await this.say(contact, [{ kind: 'text', text: 'Type the script you want read.' }]);
      return;
    }
    ...
    await this.request(
      contact,
      'VOICEOVER',
      { script: inbound.text.slice(0, 4000), language: 'en', voiceId: voice ?? undefined, style: 'natural', speed: 1 },
      'Recording…',
    );
  }
  ...
  private async copyFlow(contact: WhatsappContact, state: BotState, inbound: Inbound): Promise<void> {
    if (inbound.kind !== 'text' || inbound.text.length < 2) {
      await this.say(contact, [{ kind: 'text', text: 'Type the product name on the first line, then the details.' }]);
      return;
    }
    ...
    await this.request(
      ...
      {
        task: 'product_copy',
        productName: (first ?? '').slice(0, 120),
        details: details.slice(0, 800) || undefined,
        price,
        language: 'en',
        platforms: ['whatsapp_status', 'instagram'],
  ...
  private async credits(contact: WhatsappContact): Promise<void> {
    ...
    const price = (p: { priceByMarket: unknown }) => {
      const by = p.priceByMarket as Record<string, number>;
      const v = by[ws.currency];
      return v ? fmt(v * 100, ws.currency) : by.USD ? fmt(by.USD * 100, 'USD') : '';
    };
    await this.say(contact, [
      {
        kind: 'buttons',
      ...
      await this.say(contact, [
        {
          kind: 'text',
          preview: true,
          text: `${r.credits} credits for ${fmt(r.amountMinor, r.currency)}. Pay here and they land in your account as soon as the payment clears:\n\n${r.url}`,
        },
      ]);
    } catch (err) {
  ...
  private async unlock(contact: WhatsappContact, generationId: string): Promise<void> {
    try {
      ...
      if (!full?.url) {
        await this.say(contact, [{ kind: 'text', text: 'The full song is unlocked but I could not fetch it just now. Say "menu" and try again in a minute.' }]);
        return;
      }
      await this.say(
        contact,
        [
          { kind: 'audio', url: full.url },
          {
            kind: 'text',
            text:
              r.status === 'already_unlocked'
                ? 'Here is your full song again.'
                : `Unlocked. Here is the whole song — it is yours. ${await this.balance(contact)} credits left.`,
          },
        ],
        generationId,
      );
      ...
      if (err instanceof InsufficientCreditsError) {
        await this.notEnough(contact, 'unlock the full song');
        return;
      }
  ...
  private async notEnough(contact: WhatsappContact, what: string): Promise<void> {
    ...
    await this.say(contact, [
      {
        kind: 'buttons',
        text: `You have ${balance} credits — not enough to ${what}. Top up and I will carry on.`,
        buttons: [...packs.map((p) => ({ id: `buy:${p.code}`, title: `Buy ${p.credits} credits`.slice(0, 20) })), { id: 'menu', title: 'Not now' }],
      },
    ]);
  }
  ...
  private async request(
    ...
    capability: Parameters<GenerationService['request']>[0]['capability'],
    ...
    try {
      const { generation, balance } = await this.generations.request({
        ...
        requestedById: contact.userId!,
        capability,
        params,
        clientKey: `wa:${crypto.randomUUID()}`,
        channel: 'WHATSAPP',
      });
      await this.setState(contact, {
        flow: 'idle',
        sourceKey: (contact.state as BotState | null)?.sourceKey ?? (typeof params.sourceKey === 'string' ? params.sourceKey : undefined),
      });
      await this.say(
        contact,
        [{ kind: 'text', text: `${waiting} ${generation.credits} credits held; ${balance} left. If it fails they come straight back.` }],
        generation.id,
      );
      logger.info({ contactId: contact.id, generationId: generation.id, capability }, 'whatsapp: generation requested');
    } catch (err) {
      if (err instanceof InsufficientCreditsError) {
        await this.notEnough(contact, 'make that');
        return;
      }
  ...
  async deliver(row: Generation): Promise<void> {
    ...
    const link = await this.db.whatsappMessage.findFirst({
      where: { generationId: row.id, direction: 'OUT' },
      include: { contact: true },
      orderBy: { createdAt: 'asc' },
    });
    if (!link) {
      logger.warn({ generationId: row.id }, 'whatsapp: finished generation has no conversation to deliver to');
      return;
    }
    ...
    if (row.status === 'FAILED') {
      await this.say(
        contact,
        [
          { kind: 'text', text: `${customerMessage(row) ?? 'That did not work.'} ${row.credits} credits are back in your account.` },
          { kind: 'buttons', text: 'Try again?', buttons: [{ id: 'menu', title: 'Menu' }] },
        ],
        row.id,
      );
      return;
    ...
    switch (row.capability) {
      ...
      case 'RELIGHT': {
        ...
        if (main && urls[main.key])
          messages.push(
            row.capability === 'BACKGROUND_REMOVE'
              ? { kind: 'document', url: urls[main.key]!, filename: 'cutout.png', caption: 'Your cut-out, on transparency.' }
              : { kind: 'image', url: urls[main.key]!, caption: 'Here it is. Long-press to save or forward.' },
          );
        if (story && urls[story.key]) messages.push({ kind: 'image', url: urls[story.key]!, caption: 'And the 9:16 for your Status.' });
        messages.push({
          kind: 'buttons',
          text: `${balance} credits left. Same photo again?`,
          buttons: [
            { id: 'photo:scene', title: 'Another scene' },
            { id: 'photo:reel', title: 'Make a reel' },
            { id: 'flow:copy', title: 'Write a caption' },
          ],
        });
      ...
      case 'MUSIC': {
        ...
        messages.push({
          kind: 'buttons',
          text: `${text?.title ? `"${text.title}" — ` : ''}that is the first 30 seconds. Like it? Unlock the whole song for ${price.credits} credits and it is yours to keep and share.`,
          buttons: [
            { id: `unlock:${row.id}`, title: `Unlock · ${price.credits} cr`.slice(0, 20) },
            { id: 'flow:song', title: 'Another song' },
            { id: 'menu', title: 'Menu' },
          ],
        });
        break;
      ...
      case 'TEXT_GENERATE': {
        const t = outputs.find((o) => o.role === 'text')?.text as
          { description?: { short?: string; long?: string }; captions?: Record<string, string>; hashtags?: { broad?: string[]; local?: string[] } } | undefined;
  ...
  async contactFor(waId: string, name: string | null): Promise<WhatsappContact> {
      ...
      if (user) {
        ...
        user = await tx.user.create({
          data: {
            ...
            identities: { create: { provider: 'WHATSAPP', providerUid: waId } },
          },
        });
      }
      if (!workspaceId) {
        ...
        if (ws.wallet)
          await this.ledger.grant(
            { walletId: ws.wallet.id, amount: SIGNUP_PROMO_CREDITS, idempotencyKey: signupGrantKey(ws.id), reason: 'Welcome credits' },
            tx,
          );
    ...
    authLog('whatsapp.onboard', 'succeeded', {
      userId: result.contact.userId ?? undefined,
      workspaceId: result.contact.workspaceId ?? undefined,
  ...
  private actorFor(contact: WhatsappContact): Actor {
    return {
      ...
      surface: 'APP',
      staffRole: null,
      workspaceRoles: new Map([[contact.workspaceId!, 'OWNER']]),
      mfaLevel: 0,
      lastStepUpAt: null,
      impersonating: false,
    };
    ...
    for (const m of messages) {
      ...
      await this.db.whatsappMessage.create({
        data: {
          ...
          body: {
            ...m,
            ...(m.kind === 'image' || m.kind === 'video' || m.kind === 'audio' || m.kind === 'document' ? { url: '[signed url]' } : {}),
