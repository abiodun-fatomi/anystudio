  ...
  it('reads text, button and list replies, images (also as documents), and shrugs at the rest', () => {
    ...
    expect(
      normaliseInbound({
        ...
        from: '234',
        type: 'interactive',
        interactive: { type: 'button_reply', button_reply: { id: 'photo:scene', title: 'New scene' } },
      }),
    ).toEqual({ kind: 'choice', id: 'photo:scene', title: 'New scene' });
    expect(
      normaliseInbound({
        id: '3',
        from: '234',
        type: 'interactive',
        interactive: { type: 'list_reply', list_reply: { id: 'genre:afrobeats', title: 'Afrobeats' } },
      }),
    ).toEqual({ kind: 'choice', id: 'genre:afrobeats', title: 'Afrobeats' });
    expect(normaliseInbound({ id: '4', from: '234', type: 'image', image: { id: 'm1', mime_type: 'image/jpeg', caption: 'on a beach' } })).toEqual({
      kind: 'image',
      mediaId: 'm1',
      mime: 'image/jpeg',
      caption: 'on a beach',
    });
    expect(normaliseInbound({ id: '5', from: '234', type: 'document', document: { id: 'm2', mime_type: 'image/png' } })).toMatchObject({
      kind: 'image',
      mediaId: 'm2',
      mime: 'image/png',
    });
...
describe('outbound shapes', () => {
  it('caps buttons at three and titles at twenty characters, and sends media by link', () => {
    const b = toMeta({ kind: 'buttons', text: 'x', buttons: [1, 2, 3, 4].map((i) => ({ id: `b${i}`, title: 'a very long button title indeed' })) }) as {
    ...
    const l = toMeta({
      ...
      text: 'x',
      button: 'Choose',
      sections: [{ rows: Array.from({ length: 12 }, (_, i) => ({ id: `r${i}`, title: `Row ${i}` })) }],
