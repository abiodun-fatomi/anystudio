...
export interface InboundMessage {
  ...
  from: string;
  timestamp?: string;
  type: 'text' | 'image' | 'audio' | 'video' | 'document' | 'interactive' | 'button' | 'sticker' | 'location' | 'contacts' | 'reaction' | 'unsupported' | string;
  text?: { body?: string };
  image?: { id?: string; mime_type?: string; caption?: string; sha256?: string };
  audio?: { id?: string; mime_type?: string; voice?: boolean };
  video?: { id?: string; mime_type?: string; caption?: string };
  document?: { id?: string; mime_type?: string; filename?: string; caption?: string };
  interactive?: { type?: 'button_reply' | 'list_reply'; button_reply?: { id?: string; title?: string }; list_reply?: { id?: string; title?: string; description?: string } };
...
export type Outbound =
  ...
  | { kind: 'list'; text: string; button: string; sections: Array<{ title?: string; rows: Array<{ id: string; title: string; description?: string }> }>; header?: string; footer?: string }
...
export function normaliseInbound(m: InboundMessage): Inbound {
  switch (m.type) {
    case 'text': return { kind: 'text', text: (m.text?.body ?? '').trim() };
    case 'interactive': {
    ...
    case 'button': return { kind: 'choice', id: m.button?.payload ?? '', title: m.button?.text ?? '' };
    case 'image': return { kind: 'image', mediaId: m.image?.id ?? '', mime: m.image?.mime_type ?? 'image/jpeg', caption: m.image?.caption?.trim() || undefined };
    case 'audio': return { kind: 'audio', mediaId: m.audio?.id ?? '', mime: m.audio?.mime_type?.split(';')[0] ?? 'audio/ogg' };
    case 'video': return { kind: 'video', mediaId: m.video?.id ?? '', mime: m.video?.mime_type?.split(';')[0] ?? 'video/mp4', caption: m.video?.caption?.trim() || undefined };
    case 'document': {
    ...
    default: return { kind: 'other', type: m.type };
  }
}
