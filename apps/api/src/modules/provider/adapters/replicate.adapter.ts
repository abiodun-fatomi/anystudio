...
interface Prediction { id: string; status: 'starting' | 'processing' | 'succeeded' | 'failed' | 'canceled'; output?: unknown; error?: string; urls?: { get: string } }
...
export class ReplicateProvider extends BaseProvider {
  ...
  constructor(private readonly token: string, key: string, capability: Capability, private readonly defaultModel: string) {
  ...
  async generate(input: ProviderInput, opts: ProviderOpts): Promise<ProviderResult> {
    ...
    const first = await http<Prediction>(this.key, `https://api.replicate.com/v1/models/${model}/predictions`, { headers, body, timeoutMs: 45_000, signal: opts.signal });
    const providerJobId = first.json.id;
    opts.onProgress?.('cutting out the product', 30);

    const final = first.json.status === 'succeeded' || first.json.status === 'failed'
      ? first.json
      : await poll(
          async () => {
            const s = await http<Prediction>(this.key, first.json.urls?.get ?? `https://api.replicate.com/v1/predictions/${providerJobId}`, { headers: { authorization: headers.authorization }, timeoutMs: 15_000, signal: opts.signal });
