...
export class WatermarkDto {
  ...
  @ApiPropertyOptional({ enum: WATERMARK_POSITIONS }) @IsOptional() @IsIn(WATERMARK_POSITIONS)
  position?: (typeof WATERMARK_POSITIONS)[number];

  @ApiPropertyOptional({ minimum: 0.1, maximum: 1 }) @IsOptional() @Type(() => Number) @IsNumber() @Min(0.1) @Max(1)
  opacity?: number;
}
...
export class BrandKitDto {
  ...
  @IsOptional() @IsArray() @ArrayMaxSize(6) @Matches(/^#[0-9a-fA-F]{6}$/, { each: true })
  palette?: string[];

  @ApiPropertyOptional({ enum: FONTS }) @IsOptional() @IsIn(FONTS)
  fontDisplay?: string | null;

  @ApiPropertyOptional({ enum: FONTS }) @IsOptional() @IsIn(FONTS)
  fontBody?: string | null;

  @ApiPropertyOptional({ maxLength: 300, description: 'Free prose the copy pipeline conditions on' }) @IsOptional() @IsString() @MaxLength(300)
  tone?: string | null;

  @ApiPropertyOptional({ type: WatermarkDto }) @IsOptional() @ValidateNested() @Type(() => WatermarkDto)
  watermark?: WatermarkDto;

  @ApiPropertyOptional() @IsOptional() @IsBoolean()
  showPrice?: boolean;

  @ApiPropertyOptional({ enum: SIZE_KEYS, isArray: true }) @IsOptional() @IsArray() @ArrayMaxSize(5) @IsIn(SIZE_KEYS, { each: true })
  defaultSizes?: string[];
}
