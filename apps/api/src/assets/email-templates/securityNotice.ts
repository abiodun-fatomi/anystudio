...
const WORDS: Record<SecurityEvent, { subject: string; line: string }> = {
  password_changed: { subject: 'Your AnyStudio password was changed', line: 'The password on your AnyStudio account was just changed, and every other device was signed out.' },
  email_changed: { subject: 'The email on your AnyStudio account was changed', line: 'The email on your AnyStudio account was just changed. This address no longer signs in.' },
  mfa_enabled: { subject: 'Two-step sign-in is on', line: 'Two-step sign-in was just turned on for your AnyStudio account. From now on, signing in on a new device asks for a code from your authenticator app.' },
  mfa_disabled: { subject: 'Two-step sign-in was turned off', line: 'Two-step sign-in was just turned off for your AnyStudio account. Your password alone now signs in.' },
  recovery_codes: { subject: 'New recovery codes were made', line: 'A new set of recovery codes was just made for your AnyStudio account. The old ones no longer work.' },
...
export function securityNotice(to: string, name: string | null, event: SecurityEvent, when: Date, where: string | null, securityUrl: string): Mail {
  ...
  return {
    to,
    subject: w.subject,
    text: [greet(name), '', w.line, '', `When: ${stamp}`, '',
    ...
    html: html([greet(name), w.line, `<span style="color:#6E6575">When: ${stamp}</span>`,
      "If that was you, there's nothing to do.",
      "If it wasn't, open Security straight away, change your password and sign out everywhere."],
