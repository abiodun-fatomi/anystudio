...
export function supportTranscript(
  ...
  const rows = lines
    .map((l) => {
      ...
      return (
        `<div style="margin:0 0 14px;${mine ? 'text-align:right' : ''}">` +
  ...
  const html =
    `<div style="font-family:-apple-system,Segoe UI,Helvetica,Arial,sans-serif;font-size:15px;line-height:1.55;color:#17131A;max-width:560px">` +
