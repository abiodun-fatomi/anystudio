...
import { ProviderError, lyricsSchema, lyricsToText, LYRICS_JSON_SCHEMA, MUSIC_PREVIEW_SEC, type CapabilityParams, type LlmRequest, type Lyrics, type ProviderArtifact } from '@anystudio/shared';
...
export const musicPipeline: Pipeline = async (ctx) => {
  ...
  if (p.vocal !== 'instrumental' && !lyricsText) {
    ...
    await ctx.db.generation.update({ where: { id: ctx.row.id }, data: { input: { ...(ctx.row.input as object), lyricsWritten: lyricsText, title: p.title ?? lyrics.title } } });
  ...
  const result = await ctx.callProvider(
    {
      generationId: ctx.row.id, workspaceId: ctx.row.workspaceId, capability: 'MUSIC', files: ctx.files,
      params: { ...p, styleHints: genre.promptHints, lyricsText, title: p.title ?? lyrics?.title },
    },
    { timeoutMs: ctx.budgetMs, signal: ctx.signal, onProgress: (detail, progress) => void ctx.stage('generating', Math.max(20, Math.min(70, progress ?? 40)), detail) },
  );
  ...
  await ctx.media.recordOutput({ workspaceId: ctx.row.workspaceId, generationId: ctx.row.id, key: fullKey, kind: 'OUTPUT', mime: track.mime, bytes: bytes.byteLength, durationMs: track.durationMs });
  ...
  const artifacts: ProviderArtifact[] = [
    ...
    { text: { title: p.title ?? lyrics?.title ?? null, lyrics: lyricsText ?? null, genre: genre.name, vocal: p.vocal, language: p.language }, mime: 'application/json', role: 'text' },
  ];
  return {
    artifacts,
...
async function writeLyrics(ctx: PipelineContext, p: CapabilityParams<'MUSIC'>, genreName: string, hints: string): Promise<{ lyrics: Lyrics; costMinor?: number }> {
  const profile = (ctx.workspace.profile as Record<string, unknown> | null) ?? {};
  const language = LANGUAGE_NAME[p.language] ?? p.language;
  const targetSections = p.durationSec >= 150 ? 'intro, verse, pre-chorus, chorus, verse, chorus, bridge, chorus, outro' : p.durationSec >= 90 ? 'verse, chorus, verse, chorus, bridge, chorus' : 'verse, chorus, verse, chorus';
  const system = [
    ...
    p.vocal === 'duet' ? 'Two voices: mark lines for the second voice in a verse with "(2)".' : p.vocal === 'choir' ? 'Written for a choir: simple, repeated, singable lines.' : '',
    p.mood ? `Mood: ${p.mood}.` : '',
    `Sections in this order: ${targetSections}. Two to six lines per section; the chorus is the same words each time.`,
    profile.sells ? `The seller sells: ${String(profile.sells)}. If the brief is about their business, make it feel like a real song, not a jingle — unless they ask for a jingle.` : '',
  ...
  const parts: LlmRequest['parts'] = [{ text: [p.title ? `Title: ${p.title}` : 'Choose a short title.', `What the song is about: ${p.brief}`, 'Write the lyrics now.'].join('\n') }];
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    const req: LlmRequest = issues ? { ...request, parts: [...request.parts, { text: `Your previous answer did not fit the required structure:\n${issues}\nAnswer again, fixing exactly those problems.` }] } : request;
    const r = await ctx.callCapability('TEXT_GENERATE', { generationId: ctx.row.id, workspaceId: ctx.row.workspaceId, params: { task: 'lyrics', language: p.language }, files: {}, prompt: req }, { timeoutMs: 60_000, signal: ctx.signal });
...
export async function cutPreview(bytes: Uint8Array, ext: string, seconds: number): Promise<{ preview: Uint8Array; fullMs: number }> {
  ...
  try {
    ...
    } catch { /* unknown length is not fatal */ }
    const fadeAt = Math.max(0, seconds - 2);
    await exec('ffmpeg', ['-v', 'error', '-y', '-i', src, '-t', String(seconds), '-af', `afade=t=out:st=${fadeAt}:d=2`, '-c:a', 'libmp3lame', '-b:a', '128k', out], { maxBuffer: 32 * 1024 * 1024 });
...
const LANGUAGE_NAME: Record<string, string> = {
  en: 'English', 'en-NG': 'Nigerian English', pcm: 'Nigerian Pidgin', yo: 'Yoruba (with tone marks)', ig: 'Igbo', ha: 'Hausa', tw: 'Twi', sw: 'Swahili', zu: 'Zulu', xh: 'Xhosa',
  fr: 'French', pt: 'Portuguese', es: 'Spanish', ar: 'Arabic', hi: 'Hindi', pa: 'Punjabi', ko: 'Korean', ja: 'Japanese', zh: 'Mandarin Chinese', vi: 'Vietnamese', tr: 'Turkish', am: 'Amharic', ln: 'Lingala', wo: 'Wolof',
};
