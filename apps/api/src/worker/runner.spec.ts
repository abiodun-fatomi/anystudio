...
class MemoryMedia extends MediaService {
  ...
  override async put(key: string, bytes: Uint8Array | Buffer, mime: string): Promise<void> {
    this.objects.set(key, { bytes: new Uint8Array(bytes), mime });
  }
...
suite('GenerationRunner', () => {
  ...
  beforeAll(async () => {
    ...
    await db.providerModel.upsert({
      where: { key_capability: { key: 'stub:any', capability: 'TEXT_GENERATE' } },
      create: { key: 'stub:any', capability: 'TEXT_GENERATE', priority: 10, costPerCall: 0 },
      update: { enabled: true, breakerOpenedAt: null, config: {} },
    });
    await db.providerModel.upsert({
      where: { key_capability: { key: 'stub:any', capability: 'IMAGE_EDIT' } },
      create: { key: 'stub:any', capability: 'IMAGE_EDIT', priority: 10, costPerCall: 4 },
      update: { enabled: true, breakerOpenedAt: null, config: {} },
    });
    await db.providerModel.upsert({
      where: { key_capability: { key: 'stub:any', capability: 'IMAGE_TO_VIDEO' } },
      create: { key: 'stub:any', capability: 'IMAGE_TO_VIDEO', priority: 10, costPerCall: 80 },
      update: { enabled: true, breakerOpenedAt: null, config: {} },
    });
    await db.providerModel.upsert({
      where: { key_capability: { key: 'stub:any', capability: 'VIDEO_STITCH' } },
      create: { key: 'stub:any', capability: 'VIDEO_STITCH', priority: 10, costPerCall: 0 },
      update: { enabled: true, breakerOpenedAt: null, config: {} },
    });
    await db.providerModel.upsert({
      where: { key_capability: { key: 'stub:any', capability: 'MUSIC' } },
      create: { key: 'stub:any', capability: 'MUSIC', priority: 10, costPerCall: 0 },
      update: { enabled: true, breakerOpenedAt: null, config: {} },
    });
    await db.providerModel.upsert({
      where: { key_capability: { key: 'stub:any', capability: 'VOICEOVER' } },
      create: { key: 'stub:any', capability: 'VOICEOVER', priority: 10, costPerCall: 0 },
      update: { enabled: true, breakerOpenedAt: null, config: {} },
    });
    await db.providerModel.upsert({
      where: { key_capability: { key: 'stub:any', capability: 'DUB' } },
      create: { key: 'stub:any', capability: 'DUB', priority: 10, costPerCall: 0 },
      update: { enabled: true, breakerOpenedAt: null, config: {} },
    });
    await db.providerModel.upsert({
      where: { key_capability: { key: 'stub:any', capability: 'LIPSYNC' } },
      create: { key: 'stub:any', capability: 'LIPSYNC', priority: 10, costPerCall: 0 },
      update: { enabled: true, breakerOpenedAt: null, config: {} },
    });
    await db.musicGenre.upsert({
      where: { key: 'test-afrobeats' },
      create: {
        key: 'test-afrobeats',
        name: 'Afrobeats',
        region: 'Nigeria',
        family: 'african',
        description: 'test',
        promptHints: 'log drum, shakers, warm bass',
        languages: ['en'],
      },
      update: {},
    });
    await db.voiceProfile.upsert({
      where: { key: 'test-voice' },
      create: { key: 'test-voice', providerKey: 'stub:any', providerVoiceId: 'stub-v', name: 'Stub voice', language: 'en-NG', tags: [] },
      update: { providerKey: 'stub:any', active: true },
    });
    for (const c of [
  ...
  it('a dub with lips: priced as such, dubbed by one vendor, lips finished by a LIPSYNC vendor when the first left them alone', async () => {
    ...
    await expect(
      generations.request({
        ...
        capability: 'DUB',
        clientKey: 'dub-0',
        params: { sourceKey, targetLanguage: 'en-NG', lipsync: true },
      }),
    ).rejects.toMatchObject({ details: { consent: expect.stringContaining('permission') } });
    const { generation } = await generations.request({
      workspaceId,
      requestedById: userId,
      capability: 'DUB',
      clientKey: 'dub-1',
      params: { sourceKey, targetLanguage: 'en-NG', lipsync: true, consent: true },
    });
    ...
    const quiet = await generations.request({
      workspaceId,
      requestedById: userId,
      capability: 'DUB',
      clientKey: 'dub-2',
      params: { sourceKey, targetLanguage: 'fr', consent: true },
    });
    ...
    const nope = await generations.request({
      workspaceId,
      requestedById: userId,
      capability: 'DUB',
      clientKey: 'dub-3',
      params: { sourceKey, targetLanguage: 'yo', consent: true },
    });
  ...
  it('a lip-sync from a script records the voice first, then syncs, and keeps the words on the row', async () => {
    const sourceKey = `${workspaceId}/2026/09/uploads/clip.mp4`;
    await expect(
      generations.request({ workspaceId, requestedById: userId, capability: 'LIPSYNC', clientKey: 'ls-0', params: { sourceKey, consent: true } }),
    ).rejects.toMatchObject({ details: { script: expect.stringContaining('audio file or write a script') } });
    const { generation } = await generations.request({
      workspaceId,
      requestedById: userId,
      capability: 'LIPSYNC',
      clientKey: 'ls-1',
      params: { sourceKey, script: 'New stock **today**.', voiceId: 'test-voice', consent: true },
    });
  ...
  it('a song: lyrics are written, the full track is vaulted and locked, the preview is open, unlocking pays once and copies it out', async () => {
    ...
    const actor = {
      ...
      staffRole: null,
      workspaceRoles: new Map([[workspaceId, 'OWNER' as const]]),
      mfaLevel: 0,
      lastStepUpAt: null,
      impersonating: false,
    };
  ...
  it("a voiceover routes to the voice's own vendor and records the script alongside the audio", async () => {
    const { generation } = await generations.request({
      ...
      clientKey: 'vo-1',
      params: { script: 'Fresh ankara bags, **now** in stock.', voiceId: 'test-voice' },
    });
    expect(await runner.run(generation.id)).toBe('succeeded');
    const row = await db.generation.findUniqueOrThrow({ where: { id: generation.id } });
    ...
    await expect(
      generations
        .request({ workspaceId, requestedById: userId, capability: 'VOICEOVER', clientKey: 'vo-2', params: { script: 'x', voiceId: 'no-such-voice' } })
        .then((r) => runner.run(r.generation.id)),
  ...
  it('runs a copy generation end to end: outputs on the row, credits stand, stage narrated', async () => {
    const { generation } = await generations.request({
      ...
      clientKey: 'copy-1',
      params: { productName: 'Ankara tote', price: '₦12,000', platforms: ['instagram', 'whatsapp_status'] },
    });
    expect(await runner.run(generation.id)).toBe('succeeded');
  ...
  it('runs a branded image: composite, size variants and thumbnails land in storage and the library', async () => {
    ...
    expect(outputs.filter((o) => o.role === 'variant').map((o) => [o.size, o.width, o.height])).toEqual([
  ...
  it('refunds the whole ad when a shot fails for good', async () => {
    const { generation: parent } = await generations.request({
      ...
      capability: 'IMAGE_TO_VIDEO',
      clientKey: 'ad-2',
      params: { sourceKey: `${workspaceId}/2026/09/uploads/src.png`, prompt: 'show the bag', shots: 2 },
    });
    expect(await runner.run(parent.id)).toBe('waiting');
    const children = await db.generation.findMany({ where: { parentId: parent.id } });
    await db.providerModel.update({
      where: { key_capability: { key: 'stub:any', capability: 'IMAGE_TO_VIDEO' } },
      data: { config: { behaviour: 'fail:CONTENT_REJECTED' } },
    });
    try {
