...
export function HandoffClient() {
  ...
  useEffect(() => {
    ...
    if (!token) { window.location.replace(signInUrl(safeNext)); return; }
    api.auth.handoff(token)
      .then((r) => {
        if (r.status === 'signed_in') router.replace(safeNext ?? r.next);
        else { setFailed(true); window.location.replace(signInUrl(safeNext)); }
      })
      .catch(() => { setFailed(true); window.location.replace(signInUrl(safeNext)); });
  }, [params, router]);

  return (
    <main style={{ minHeight: '100dvh', display: 'grid', placeItems: 'center', color: 'var(--fg-2, #9a9aa6)', fontSize: 14 }} aria-live="polite">
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <span aria-hidden="true" style={{ width: 14, height: 14, borderRadius: '50%', border: '2px solid currentColor', borderRightColor: 'transparent', animation: 'as-spin .8s linear infinite', display: 'inline-block' }} />
