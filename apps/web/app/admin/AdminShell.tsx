...
const NAV: Array<{ href: string; label: string; icon: IconName; min?: StaffRole }> = [
  ...
  { href: '/admin/economics', label: 'Economics', icon: 'credits', min: 'SUPERADMIN' },
  { href: '/admin/billing', label: 'Invoicing', icon: 'credits' },
  { href: '/admin/careers', label: 'Careers', icon: 'user', min: 'SUPPORT' },
  { href: '/admin/leads', label: 'Platform leads', icon: 'bell', min: 'SUPPORT' },
  { href: '/admin/providers', label: 'Providers & prices', icon: 'settings', min: 'OPERATOR' },
  { href: '/admin/templates', label: 'Templates', icon: 'studio', min: 'ADMIN' },
...
export function AdminShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    api.auth
      ...
      .then((m) => {
        ...
        if (!m.staffRole) {
          setDenied('This account has no staff access. Ask an admin to grant it, then sign in again with your second factor.');
          setMe(null);
          return;
        }
        setMe(m);
  ...
  if (!me)
    return (
      <div className={styles.frame}>
        <div className={styles.main}>
          <EmptyState icon={<Icon.lock />} title="Staff console" body={denied} actions={<Button href="/login?next=/admin">Sign in again</Button>} />
        </div>
      </div>
    );
        ...
        <div className={styles.frame}>
          <aside className={styles.rail} aria-label="Console">
            <div className={styles.brand}>
              ...
              <span className={styles.env}>{typeof window !== 'undefined' ? window.location.host.replace(/^admin\./, '') : ''}</span>
            </div>
            <nav className={styles.nav}>
              {NAV.filter((n) => !n.min || atLeast(n.min)).map((n) => (
                <Link
                  ...
                  aria-current={(n.href === '/admin' ? path === '/admin' : path.startsWith(n.href)) ? 'page' : undefined}
                >
                  {Icon[n.icon]({ width: 18, height: 18 })}
                  <span>{n.label}</span>
                </Link>
              ))}
            </nav>
            <div className={styles.foot}>
              ...
              <a href={siblingOrigin(window.location.host, 'app')} className={styles.small}>
                Customer app →
              </a>
              <button type="button" className={styles.small} onClick={() => void signOut()}>
                Sign out
              </button>
            </div>
          </aside>
