  ...
  return (
    <div className="rise">
      <PageHeader
        title="Help chats"
        lede={
          data ? `${data.counts.open} open · ${data.counts.needsHuman} waiting for a person. The assistant answers first; step in from a chat.` : 'Loading…'
        }
      />
      <div className={styles.toolbar}>
        <SegmentedControl<Filter>
          ...
          onChange={setFilter}
          items={[
            { id: 'needs_human', label: `Needs a person${data?.counts.needsHuman ? ` (${data.counts.needsHuman})` : ''}` },
            { id: 'open', label: 'Open' },
      ...
      {data === null ? (
        ...
        <p style={{ color: 'var(--muted)' }}>{filter === 'needs_human' ? 'Nobody is waiting. The assistant has it.' : 'No chats here.'}</p>
      ) : (
        <Table>
          <thead>
            <tr>
          ...
          <tbody>
            {data.rows.map((r) => (
              <tr
                ...
                onClick={() => {
                  window.location.href = `/admin/support/${r.id}`;
                }}
              >
                <td>
                  <Link href={`/admin/support/${r.id}`} onClick={(e) => e.stopPropagation()}>
                    <strong>{r.user.name ?? r.user.email ?? 'Someone'}</strong>
                  </Link>
                ...
                <td>
                  {r.topic ?? <span style={{ color: 'var(--muted)' }}>—</span>}
                  {r.page && (
                    <div className={styles.mono} style={{ color: 'var(--muted)' }}>
                      {r.page}
                    </div>
                ...
                <td style={{ maxWidth: 360 }}>
                  {r.last ? (
                    <>
                      <span style={{ color: 'var(--muted)', fontSize: 'var(--t-1)' }}>{ROLE[r.last.role]} · </span>
                      {r.last.text}
                    </>
                  ) : (
                    <span style={{ color: 'var(--muted)' }}>Nothing said yet</span>
                  )}
                </td>
                ...
                <td style={{ whiteSpace: 'nowrap', color: 'var(--muted)' }}>
                  ...
                  <div className={styles.mono}>{r.messageCount} msgs</div>
                </td>
              </tr>
            ))}
          </tbody>
