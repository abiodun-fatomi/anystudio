...
export default function AuditPage() {
  const [userId, setUserId] = useState(''); const [type, setType] = useState('');
  ...
  const search = useCallback(async (after?: string) => {
    ...
    try { const r = await api.admin.audit({ userId: userId.trim() || undefined, type: type.trim() || undefined, cursor: after }); setRows((cur) => (after && cur ? [...cur, ...r.events] : r.events)); setCursor(r.nextCursor); } catch { setRows([]); }
  }, [userId, type]);
  ...
  return (
    <div className="rise">
      ...
      {rows === null ? <Skeleton height={240} /> : (
        <>
          <Table>
            <thead><tr><th>When</th><th>Event</th><th>Who</th><th>Surface</th><th>IP</th><th>Detail</th></tr></thead>
            <tbody>{rows.map((e) => <tr key={e.id}><td className={tableCell.shrink}>{new Date(e.createdAt).toLocaleString()}</td><td className={styles.mono}>{e.type}</td><td>{e.user ? <a href={`/admin/customers/${e.userId}`}>{e.user.name ?? e.user.email ?? e.user.phone}</a> : '—'}</td><td>{e.surface ?? '—'}</td><td className={styles.mono}>{e.ip ?? '—'}</td><td className={styles.mono} style={{ color: 'var(--muted)' }}>{e.detail ? JSON.stringify(e.detail).slice(0, 140) : ''}</td></tr>)}</tbody>
          </Table>
