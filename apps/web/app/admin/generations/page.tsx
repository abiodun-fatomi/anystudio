...
function Generations() {
  ...
  const show = (id: string) =>
    api.admin
      .generation(id)
      .then(setOpen)
      .catch(() => toast({ title: 'Could not load it', tone: 'danger' }));
  const act = async () => {
    if (!open || !action) return;
    setBusy(true);
    try {
      if (action === 'fail') await api.admin.failGeneration(open.id, reason.trim());
      else await api.admin.refundGeneration(open.id, reason.trim());
      toast({ title: action === 'fail' ? 'Ended; credits returned' : 'Refunded', tone: 'ok' });
      setAction(null);
      setReason('');
      void show(open.id);
      void search();
    ...
    <div className="rise">
      ...
      >
        ...
        <Select
          ...
          options={['', 'QUEUED', 'RUNNING', 'SUCCEEDED', 'FAILED', 'CANCELLED'].map((s) => ({ value: s, label: s || 'Any status' }))}
        />
        <Select
          label="Capability"
          value={capability}
          onChange={(e) => setCapability(e.target.value)}
          options={CAPS.map((c) => ({ value: c, label: c || 'Any capability' }))}
        />
          ...
          <Table>
            <thead>
              <tr>
                ...
                <th className={tableCell.num}>Vendor cost</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((g) => (
                <tr key={g.id} className={styles.clickRow} onClick={() => void show(g.id)}>
                  ...
                  <td>
                    <strong>{g.title ?? g.capability}</strong>
                    <div className={styles.mono} style={{ color: 'var(--muted)' }}>
                      {g.capability} · {g.id.slice(0, 8)}
                    </div>
                  </td>
                  <td>
                  ...
                  <td style={{ fontSize: 'var(--t-1)', color: 'var(--muted)', maxWidth: 280 }}>
                    {g.failureKind ? (
                      <>
                        <span className={styles.danger}>{g.failureKind}</span> {g.failureReason?.slice(0, 100)}
                      </>
                    ) : (
                      ''
                    )}
      ...
      <Dialog
        ...
        title={open ? (open.title ?? open.capability) : ''}
        wide
        sheet="right"
        footer={
          open && atLeast('OPERATOR') ? (
            <>
              {(open.status === 'RUNNING' || open.status === 'QUEUED') && (
                <Button variant="danger" onClick={() => setAction('fail')}>
                  End it, credits back
                </Button>
              )}
              {open.status === 'SUCCEEDED' && open.credits > 0 && (
        ...
        {open && (
          <div style={{ display: 'grid', gap: 'var(--s-4)' }}>
            <dl className={styles.kv}>
              ...
              <dt>Workspace</dt>
              <dd>
                <a href={`/admin/workspaces/${open.workspaceId}`}>{open.workspace.name}</a> ({open.workspace.type.toLowerCase()})
              </dd>
              <dt>Asked by</dt>
              <dd>
                <a href={`/admin/customers/${open.requestedBy.id}`}>{open.requestedBy.name ?? open.requestedBy.email ?? open.requestedBy.phone}</a> via{' '}
                {open.channel.toLowerCase()}
              </dd>
              <dt>Status</dt>
              <dd>
                {open.status} · {open.stage ?? ''} · attempt {open.attempts}
              </dd>
              <dt>Provider</dt>
              <dd className={styles.mono}>
                {open.providerKey ?? '—'} {open.providerJobId ? `· job ${open.providerJobId}` : ''}
              </dd>
              <dt>Credits</dt>
              <dd>
                {open.credits} charged{open.providerCostMinor != null ? ` · vendor ${(open.providerCostMinor / 100).toFixed(2)}` : ''}
              </dd>
              <dt>Timing</dt>
              <dd>
                {new Date(open.createdAt).toLocaleString()} → {open.finishedAt ? new Date(open.finishedAt).toLocaleString() : '…'}
              </dd>
              {open.failureKind && (
                <>
                  <dt>Failure</dt>
                  <dd className={styles.danger}>
                    {open.failureKind}: {open.failureReason}
                  </dd>
                </>
              )}
            </dl>
            {action && (
              <div className={styles.card}>
                <div className={styles.cardTitle}>
                  {action === 'fail' ? 'End this generation and return the credits' : 'Refund the credits as goodwill (outputs stay)'}
                </div>
                ...
                <div style={{ display: 'flex', gap: 'var(--s-2)' }}>
                  ...
                  <Button variant="danger" onClick={act} loading={busy} disabled={reason.trim().length < 4}>
                    Confirm
                  </Button>
                </div>
              </div>
            ...
            <div>
              ...
              <pre className="json">{JSON.stringify(open.input, null, 2)}</pre>
            </div>
            <div>
              <div className={styles.cardTitle}>Outputs</div>
              <pre className="json">{JSON.stringify(open.outputs, null, 2)}</pre>
            </div>
            {open.children.length > 0 && (
              <div>
                ...
                <pre className="json">
                  {JSON.stringify(
                    open.children.map((c) => {
                      const x = c as { id: string; status: string; providerKey: string | null; failureKind: string | null };
                      return { id: x.id, status: x.status, providerKey: x.providerKey, failureKind: x.failureKind };
                    }),
                    null,
                    2,
                  )}
