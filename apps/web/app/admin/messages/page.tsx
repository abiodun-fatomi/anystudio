...
export default function MessagesPage() {
  ...
  const create = async (publish: boolean) => {
    ...
    try {
      ...
      toast({ title: publish ? 'Published' : 'Saved as draft', tone: 'ok' });
      setOpen(false);
      setForm({ title: '', body: '', href: '', audience: 'ALL', expiresAt: '' });
      load();
    } catch (e) {
  ...
  const setPublished = async (m: AdminMessage, published: boolean) => {
    try {
      await api.admin.updateMessage(m.id, { published });
      load();
    } catch (e) {
      toast({ title: 'Refused', body: e instanceof Error ? e.message : undefined, tone: 'danger' });
    }
  ...
  return (
    <div className="rise">
      <PageHeader
        ...
        lede="One row, read by everyone in the audience, in their bell. Publish when ready; unpublish to pull it back."
        actions={<Button onClick={() => setOpen(true)}>New message</Button>}
      />
      {rows === null ? (
        <Skeleton height={200} />
      ) : rows.length === 0 ? (
        <p style={{ color: 'var(--muted)' }}>Nothing written yet.</p>
      ) : (
        <Table>
          <thead>
            <tr>
              ...
              <th>Audience</th>
              <th>State</th>
              <th className={tableCell.num}>Read by</th>
              <th />
            </tr>
          </thead>
          <tbody>
            {rows.map((m) => (
              <tr key={m.id}>
                <td>
                  ...
                  <div style={{ fontSize: 'var(--t-1)', color: 'var(--muted)' }}>{m.body.slice(0, 120)}</div>
                </td>
                <td>{m.audience.toLowerCase()}</td>
                <td>
                  {m.publishedAt ? (
                    <span className={styles.pill} data-tone="ok">
                      live since {new Date(m.publishedAt).toLocaleDateString()}
                    </span>
                  ) : (
                    <span className={styles.pill}>draft</span>
                  )}
                  {m.expiresAt && <div style={{ fontSize: 'var(--t-1)', color: 'var(--muted)' }}>until {new Date(m.expiresAt).toLocaleDateString()}</div>}
                </td>
                <td className={tableCell.num}>{m._count?.reads ?? 0}</td>
                <td style={{ whiteSpace: 'nowrap' }}>
                  <Button variant="ghost" size="sm" onClick={() => setPublished(m, !m.publishedAt)}>
      ...
      <Dialog
        ...
        footer={
          <>
            ...
            <Button variant="ghost" onClick={() => create(false)} loading={busy} disabled={form.title.trim().length < 2 || form.body.trim().length < 2}>
              Save draft
            </Button>
            <Button onClick={() => create(true)} loading={busy} disabled={form.title.trim().length < 2 || form.body.trim().length < 2}>
              Publish now
            </Button>
          </>
        }
        ...
        <div style={{ display: 'grid', gap: 'var(--s-4)' }}>
          ...
          <Select
            ...
            options={[
              ...
              { value: 'BUSINESS', label: 'Business workspaces' },
