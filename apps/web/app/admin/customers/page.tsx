...
export default function CustomersPage() { return <Suspense fallback={null}><Customers /></Suspense>; }
...
function Customers() {
  ...
  const search = async (after?: string) => {
    ...
    try { const r = await api.admin.customers(q.trim(), after); setRows((cur) => (after && cur ? [...cur, ...r.customers] : r.customers)); setCursor(r.nextCursor); } catch { setRows([]); }
  };
  ...
  return (
    <div className="rise">
      ...
      {rows === null ? <Skeleton height={200} /> : (
        <>
          <Table>
            <thead><tr><th>Person</th><th>Contact</th><th>Status</th><th>Workspaces</th><th>Joined</th><th>Last sign-in</th></tr></thead>
            <tbody>{rows.map((u) => (
              <tr key={u.id} className={styles.clickRow} onClick={() => router.push(`/admin/customers/${u.id}`)}>
                <td><strong>{u.name ?? '—'}</strong><div className={styles.mono} style={{ color: 'var(--muted)' }}>{u.id.slice(0, 8)}</div></td>
                <td>{u.email ?? ''}{u.email && u.phone ? ' · ' : ''}{u.phone ?? ''}</td>
                <td><span className={styles.pill} data-tone={u.status === 'ACTIVE' ? 'ok' : u.status === 'SUSPENDED' ? 'danger' : undefined}>{u.status}</span></td>
                <td style={{ fontSize: 'var(--t-1)' }}>{u.workspaces.map((w) => `${w.name} (${w.type.toLowerCase()}, ${w.role.toLowerCase()})`).join(' · ') || '—'}</td>
