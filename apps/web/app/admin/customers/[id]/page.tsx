...
export default function CustomerPage() {
  ...
  const act = async () => {
    ...
    setBusy(true);
    try {
      await api.admin.suspend(id, reason.trim(), ask === 'suspend');
      toast({ title: ask === 'suspend' ? 'Suspended — sessions ended' : 'Reinstated', tone: 'ok' });
      setAsk(null);
      setReason('');
      load();
    } catch (e) {
  ...
  return (
    <div className="rise">
      <PageHeader
        title={u.name ?? u.email ?? u.phone ?? 'Customer'}
        lede={`${u.email ?? ''}${u.email && u.phone ? ' · ' : ''}${u.phone ?? ''}${u.phoneIsWhatsApp ? ' (WhatsApp)' : ''}`}
        actions={
          atLeast('OPERATOR') && u.id !== me.user.id ? (
            u.status === 'SUSPENDED' ? (
              <Button variant="ghost" onClick={() => setAsk('unsuspend')}>
                Reinstate
              </Button>
            ) : (
              <Button variant="danger" onClick={() => setAsk('suspend')}>
                Suspend
              </Button>
            )
          ) : undefined
      ...
      <div className={styles.two}>
        <div className={styles.card}>
          ...
          <dl className={styles.kv}>
            <dt>Status</dt>
            <dd>
              <span className={styles.pill} data-tone={u.status === 'ACTIVE' ? 'ok' : 'danger'}>
                {u.status}
              </span>
              {u.deleteRequestedAt ? <span className={styles.warn}> · deletion requested {new Date(u.deleteRequestedAt).toLocaleDateString()}</span> : null}
            </dd>
            <dt>Id</dt>
            <dd className={styles.mono}>{u.id}</dd>
            <dt>Joined</dt>
            <dd>{new Date(u.createdAt).toLocaleString()}</dd>
            <dt>Last sign-in</dt>
            <dd>{u.lastLoginAt ? new Date(u.lastLoginAt).toLocaleString() : 'never'}</dd>
            <dt>Verified</dt>
            <dd>{[u.emailVerifiedAt ? 'email' : null, u.phoneVerifiedAt ? 'phone' : null].filter(Boolean).join(', ') || 'nothing yet'}</dd>
            <dt>Sign-in methods</dt>
            <dd>{u.identities.map((i) => i.provider.toLowerCase()).join(', ') || '—'}</dd>
            <dt>Two-step</dt>
            <dd>{u.mfaFactors.filter((f) => f.confirmedAt).length ? `${u.mfaFactors.filter((f) => f.confirmedAt).length} factor(s)` : 'off'}</dd>
            {u.staffGrants.length > 0 && (
              <>
                <dt>Staff</dt>
                <dd className={styles.warn}>{u.staffGrants.map((g) => g.role).join(', ')}</dd>
              </>
            )}
              ...
              {d.workspaces.map((w) => (
                <tr key={w.id}>
                  <td>
                    ...
                    {w.deletedAt && <span className={styles.danger}> (deleted)</span>}
                  </td>
                  <td>{w.type.toLowerCase()}</td>
                  <td>{w.role.toLowerCase()}</td>
      ...
      <div className={styles.two} style={{ marginTop: 'var(--s-4)' }}>
        <div className={styles.card}>
          ...
          {d.generations.length === 0 ? (
            <p style={{ color: 'var(--muted)', fontSize: 'var(--t-2)' }}>None yet.</p>
          ) : (
            <Table>
              <thead>
                <tr>
          ...
          {d.payments.length === 0 ? (
            ...
            <Table>
              ...
              <tbody>
                {d.payments.map((p) => (
                  <tr key={p.id}>
                    <td className={tableCell.shrink}>{new Date(p.createdAt).toLocaleDateString()}</td>
      ...
      <Dialog
        open={ask !== null}
        onClose={() => setAsk(null)}
        title={ask === 'suspend' ? 'Suspend this account?' : 'Reinstate this account?'}
        description={
          ask === 'suspend' ? 'They can still sign in and read, but cannot generate or buy. Every current session ends.' : 'They can generate and buy again.'
        }
        locked={busy}
        footer={
          <>
            ...
            <Button variant={ask === 'suspend' ? 'danger' : 'primary'} onClick={act} loading={busy} disabled={reason.trim().length < 4}>
              {ask === 'suspend' ? 'Suspend' : 'Reinstate'}
            </Button>
          </>
        }
      >
