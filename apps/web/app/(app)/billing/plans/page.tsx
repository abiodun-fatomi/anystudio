...
export default function PlansPage() {
  ...
  const load = useCallback(async () => {
    try {
      setCat(await api.billing.catalogue(workspace.id));
      setError(null);
    } catch (e) {
  ...
  return (
    <div className="rise">
      <PageHeader
        title="Add credits"
        lede={`Prices in ${cat?.currency ?? workspace.currency}, fixed for this workspace. A plan is the better rate; a pack is for when you need more this month.`}
        crumbs={[{ label: 'Credits', href: '/billing' }, { label: 'Add credits' }]}
        actions={
          balance !== null ? (
            <span className="mono" style={{ color: 'var(--muted)' }}>
              {balance.toLocaleString()} credits now
            </span>
          ) : undefined
        }
      />
      ...
      {!error && cat && !cat.available && (
        <div className={styles.notice}>
          <strong>Payments in {cat.currency} are not switched on yet.</strong>
          <span>Message support and we will add credits by hand until they are.</span>
        </div>
      )}
      {!canBuy && (
        <div className={styles.notice}>
          <strong>Only the owner, an admin or the billing contact can buy.</strong>
          <span>Ask them, or ask to be made the billing contact under Settings → Workspace.</span>
        </div>
      )}
      ...
      <section className={styles.section} aria-labelledby="plans">
        <div className={styles.sectionHead}>
          <div>
            ...
            <p>Credits every month. Unused plan credits roll into the next month for as long as the plan runs.</p>
          </div>
          {yearly && (
            <SegmentedControl
              ...
              items={[
                { id: 'month', label: 'Monthly' },
                {
                  ...
                  label: (
                    <>
                      Yearly <Badge tone="ok">2 months free</Badge>
                    </>
                  ),
                },
              ]}
      ...
      <section className={styles.section} aria-labelledby="packs">
        <div className={styles.sectionHead}>
          <div>
            ...
            <p>One-time credits that never expire. Good for a launch week or a plan that ran dry.</p>
          </div>
        </div>
        <div className={styles.grid}>
          {cat === null
      ...
      <p className={styles.fine}>
        Payments are taken by{' '}
        {cat?.provider === 'PADDLE'
          ? 'Paddle, our merchant of record, which handles VAT and receipts'
          : cat?.provider === 'FLUTTERWAVE'
            ? 'Flutterwave — cards, bank transfer, USSD and mobile money'
            : 'our payment partner'}
