...
const PAY_STATUS: Record<string, { label: string; tone?: 'ok' | 'warn' | 'danger' }> = { SUCCEEDED: { label: 'Paid', tone: 'ok' }, FAILED: { label: 'Failed', tone: 'danger' }, REFUNDED: { label: 'Refunded', tone: 'warn' }, PENDING: { label: 'Pending' } };
...
export default function BillingPage() {
  ...
  const cancel = async () => {
    setCancelling(true);
    try { setSub(await api.billing.cancel(workspace.id)); toast({ title: 'Plan will stop at the end of the period', body: 'Everything you paid for stays until then.', tone: 'ok' }); }
    catch (e) { toast({ title: 'Could not cancel', body: e instanceof Error ? e.message : undefined, tone: 'danger' }); }
  ...
  return (
    <div className="rise">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: 'var(--s-4)' }}>
        ...
        <Card>
          {sub === undefined ? <Skeleton height={56} /> : sub ? (
            <Stat label="Plan" value={PLAN_WORDS[sub.planCode]?.name ?? sub.planCode} sub={sub.cancelAtPeriodEnd ? `ends ${sub.currentPeriodEnd ? new Date(sub.currentPeriodEnd).toLocaleDateString() : 'at period end'}` : sub.status === 'PAST_DUE' ? 'payment overdue — update your card' : `renews ${sub.currentPeriodEnd ? new Date(sub.currentPeriodEnd).toLocaleDateString() : 'monthly'} · ${sub.interval === 'year' ? 'yearly' : 'monthly'}`} />
          ) : <Stat label="Plan" value="Free" sub="pay as you go with packs" />}
          {sub && !sub.cancelAtPeriodEnd && canBuy && <div style={{ marginTop: 'var(--s-2)' }}><Button variant="link" size="sm" onClick={() => setCancelOpen(true)}>Cancel plan</Button></div>}
          {(!sub || sub.cancelAtPeriodEnd) && canBuy && <div style={{ marginTop: 'var(--s-2)' }}><Button variant="link" size="sm" href="/billing/plans">{sub ? 'Choose another plan' : 'See plans'}</Button></div>}
        </Card>
      </div>

      <Section title="Payments">
        {payments === null ? <Skeleton height={44} /> : payments.length === 0 ? <p style={{ color: 'var(--muted)', fontSize: 'var(--t-2)' }}>No payments yet. Every purchase shows here with its reference and the amount actually charged.</p> : (
          <Table>
            <thead><tr><th>When</th><th>What</th><th>Status</th><th>Reference</th><th className={tableCell.num}>Credits</th><th className={tableCell.num}>Charged</th></tr></thead>
            <tbody>
              {payments.map((p) => (
                <tr key={p.id}>
                  <td className={tableCell.shrink}>{when(p.createdAt)}</td>
                  <td>{p.kind === 'PACK' ? 'Credit pack' : p.kind === 'RENEWAL' ? `${PLAN_WORDS[p.itemCode]?.name ?? p.itemCode} renewal` : `${PLAN_WORDS[p.itemCode]?.name ?? p.itemCode} plan${p.interval === 'year' ? ', yearly' : ''}`}<span style={{ color: 'var(--muted)' }}> · {PROVIDER[p.provider] ?? p.provider}</span></td>
                  <td className={tableCell.shrink}><Badge tone={PAY_STATUS[p.status]?.tone}>{PAY_STATUS[p.status]?.label ?? p.status}</Badge></td>
                  <td style={{ color: 'var(--muted)', fontSize: 'var(--t-1)', fontFamily: 'var(--f-mono)' }}>{p.reference}</td>
                  <td className={tableCell.num}>{p.status === 'SUCCEEDED' ? `+${p.credits.toLocaleString()}` : p.status === 'REFUNDED' ? `−${p.credits.toLocaleString()}` : '—'}</td>
