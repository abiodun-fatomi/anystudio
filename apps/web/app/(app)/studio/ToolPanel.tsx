  ...
  return (
    <section className={`${styles.pane}`} aria-label={`${tool.label} settings`}>
      <div className={styles.paneBody}>
        <div className={styles.panel}>
          ...
          <div className={styles.fields}>
            {tool.fields.filter((f) => !f.showIf || f.showIf(values)).map((f) => <FieldControl key={f.key} field={f} value={values[f.key]} onChange={(v) => onChange(f.key, v)} />)}
          </div>
      ...
      <div className={styles.generateBar}>
        ...
        <Button loading={busy} disabled={blocked} onClick={() => quote && onGenerate(quote)}>{label}</Button>
      </div>
    </section>
  );
}
...
const EMPTY_HINT: Record<string, string> = { voices: 'No voice vendor is configured in this environment yet.', languages: 'No dubbing vendor is configured in this environment yet.', sourceLanguages: 'No dubbing vendor is configured in this environment yet.', genres: 'The catalogue is empty.' };
function CatalogueField({ field, value, onChange }: { field: Extract<Field, { kind: 'catalogue' }>; value: string; onChange: (v: unknown) => void }) {
  ...
  useEffect(() => {
    let live = true;
    const done = (o: Option[]) => { if (live) setOptions(o); };
    const fail = () => { if (live) setOptions([]); };
    if (field.source === 'genres') {
      catalogueCache.genres ??= api.audio.genres();
      catalogueCache.genres.then((gs) => done(gs.map((g) => ({ value: g.key, label: g.name, sub: `${g.region} · ${g.description}`, keywords: `${g.family} ${g.region} ${g.languages.join(' ')}` })))).catch(fail);
    } else if (field.source === 'voices') {
      catalogueCache.voices ??= api.audio.voices();
      catalogueCache.voices.then((vs) => done(vs.map((v) => ({ value: v.key, label: v.name, sub: [v.accent ? `${v.accent} ${v.language.startsWith('en') ? 'English' : v.language}` : v.language, v.gender, ...v.tags].filter(Boolean).join(' · '), keywords: `${v.language} ${v.accent ?? ''} ${v.gender ?? ''} ${v.tags.join(' ')} ${v.provider}` })))).catch(fail);
    } else {
      catalogueCache.languages ??= api.audio.dubLanguages();
      catalogueCache.languages.then((d) => {
        if (field.source === 'sourceLanguages') done(d.sources.map((l) => ({ value: l.code, label: l.name })));
        else { done(d.languages.map((l) => ({ value: l.code, label: l.name, sub: l.lipsync ? `${l.region} · lips can be matched` : l.region, keywords: `${l.region} ${l.code}` }))); if (live) setNote(d.missing); }
      }).catch(fail);
    }
    return () => { live = false; };
  }, [field.source]);
  ...
  if (field.source === 'sourceLanguages') return <Select label={field.label} options={(options ?? []).map((o) => ({ value: o.value, label: o.label }))} value={value} onChange={(e) => onChange(e.target.value)} />;
  return <Combobox label={field.label} hint={note ?? field.hint} options={options ?? []} value={value} onChange={onChange} placeholder={options ? 'Search…' : 'Loading…'} emptyText={field.source === 'languages' ? 'Nothing matches — try a region or a country' : "Nothing matches — try a region or a word like 'church' or 'club'"} />;
}
...
function FileField({ field, value, onChange }: { field: Extract<Field, { kind: 'file' }>; value: string; onChange: (v: unknown) => void }) {
  ...
  const accept = field.accept === 'video' ? 'video/mp4,video/quicktime,video/webm,.mp4,.mov,.webm' : 'audio/mpeg,audio/mp4,audio/wav,audio/ogg,audio/x-m4a,.mp3,.m4a,.wav,.ogg';

  // A key that arrived without a file (a "do it again", a prefill) still gets its player.
  useEffect(() => {
    let live = true;
    if (!value) { setUrl(null); return; }
    api.media.urls(workspace.id, [value]).then(({ urls }) => { if (live) setUrl(urls[value] ?? null); }).catch(() => undefined);
    return () => { live = false; };
  }, [workspace.id, value]);

  const pick = async (file: File | undefined) => {
    if (!file) return;
    setError(null); setName(file.name); setPct(0);
    try {
      const asset = await uploadFile(workspace.id, file, (p) => setPct(p.pct));
      setPct(null);
      onChange(asset.key);
    } catch (err) {
      setPct(null); setError(err instanceof Error ? err.message : 'Upload failed');
    }
  };
      ...
      {value && url ? (
        <div className={styles.fileIn}>
          ...
          <div className={styles.fileRow}>
            ...
            <Button variant="ghost" size="sm" onClick={() => input.current?.click()}>Change</Button>
          </div>
        </div>
      ) : (
        <button type="button" className={styles.fileDrop} onClick={() => input.current?.click()} disabled={pct !== null} aria-label={field.label}
          ...
          {pct !== null ? (
            <div style={{ width: '100%' }}><div className={styles.uploadName}>{name}</div><Progress value={pct} label={pct < 100 ? 'Uploading' : 'Checking'} /></div>
          ) : (
            <><strong>{value ? 'Loading your file…' : `Add ${field.accept === 'video' ? 'a video' : 'an audio file'}`}</strong><span>{error ?? field.hint ?? 'Drop it here or tap to choose.'}</span></>
          )}
        </button>
      )}
    ...
    case 'consent':
      return (
        <label className={styles.consent}>
          ...
          <span><strong>{field.label}</strong>{field.hint && <span>{field.hint}</span>}</span>
        </label>
      );
    case 'sizes': {
