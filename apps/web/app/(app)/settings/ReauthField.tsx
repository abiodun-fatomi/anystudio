...
export function ReauthField({ profile, value, onChange, errors, autoFocus }: { profile: Profile | null; value: ReauthValue; onChange: (v: ReauthValue) => void; errors?: Record<string, string>; autoFocus?: boolean }) {
  ...
  if (kind === 'password') {
    return <Input label="Your current password" type="password" autoComplete="current-password" value={value.currentPassword ?? ''} onChange={(e) => onChange({ ...value, currentPassword: e.target.value })} error={errors?.currentPassword} autoFocus={autoFocus} />;
  }
  if (kind === 'code') {
