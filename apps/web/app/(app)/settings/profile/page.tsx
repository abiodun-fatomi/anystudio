...
const LOCALES = [
  { value: '', label: 'Same as my browser' },
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    try {
      browserZone.current = Intl.DateTimeFormat().resolvedOptions().timeZone;
    } catch {
      /* fine */
    }
  }, []);
  ...
  if (!profile && error)
    return (
      <div className={styles.group}>
        <LoadError what="your profile" message={error} onRetry={() => void reload()} />
      </div>
    );
  if (!profile)
    return (
    ...
    <>
      <section className={styles.group} aria-labelledby="p-you">
        <div className={styles.groupHead}>
          <div>
            <h2 id="p-you" className={styles.groupTitle}>
              You
            </h2>
            <p className={styles.groupLede}>Your name and picture appear to people you work with.</p>
          </div>
        </div>
        <div className={styles.avatarRow}>
          ...
          <div style={{ display: 'flex', gap: 'var(--s-2)', flexWrap: 'wrap' }}>
            ...
            />
            <Button variant="subtle" size="sm" loading={uploading} onClick={() => fileInput.current?.click()}>
              {profile.avatarKey ? 'Change picture' : 'Add a picture'}
            </Button>
            {profile.avatarKey && (
              <Button
                variant="ghost"
                size="sm"
                onClick={() => void api.account.updateProfile({ avatarKey: null }).then(() => Promise.all([reload(), refreshMe()]))}
              >
                Remove
              </Button>
            )}
        ...
        <div className={styles.grid2}>
          ...
          <Select
            label="Time zone"
            options={[{ value: '', label: 'Same as my browser' }, ...zones.map((z) => ({ value: z, label: z.replace('_', ' ') }))]}
            value={v.timezone}
      ...
      <section className={styles.group} aria-labelledby="p-email">
        <div className={styles.groupHead}>
          <div>
            <h2 id="p-email" className={styles.groupTitle}>
              Email
            </h2>
            <p className={styles.groupLede}>Where sign-in links, receipts and security notices go.</p>
          </div>
          {profile.emailVerifiedAt ? (
            <span className={styles.on}>✓ Verified</span>
          ) : (
            <Button
              variant="subtle"
              size="sm"
              onClick={() =>
                void api.auth.resendVerification().then(() => toast({ title: 'Sent', body: 'Check your inbox for the confirmation link.', tone: 'ok' }))
              }
            >
              Resend confirmation
            </Button>
        ...
        >
          ...
          <div className={styles.saveBar} style={{ gridColumn: '1 / -1' }}>
            <Button type="submit" variant="subtle" loading={sending} disabled={!newEmail.includes('@')}>
