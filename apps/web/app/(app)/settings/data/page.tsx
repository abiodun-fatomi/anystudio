...
export default function DataPage() {
  ...
  if (!profile && error)
    return (
      <div className={styles.group}>
        <LoadError what="your data settings" message={error} onRetry={() => void reload()} />
      </div>
    );
  if (!profile)
    return (
  ...
  const exportAll = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `anystudio-export-${new Date().toISOString().slice(0, 10)}.json`;
      a.click();
      setTimeout(() => URL.revokeObjectURL(url), 10_000);
    } catch (e) {
      toast({ title: 'Could not export', body: e instanceof Error ? e.message : undefined, tone: 'danger' });
    } finally {
      setExporting(false);
    }
  };
  const requestDeletion = async () => {
    ...
    try {
      ...
      toast({
        ...
        body: `Everything goes on ${new Date(r.deleteOn).toLocaleDateString()}. Sign in before then to keep it.`,
        tone: 'warn',
        durationMs: 10_000,
      });
    } catch (e) {
  ...
  const keep = async () => {
    ...
    try {
      ...
      await Promise.all([reload(), refreshMe()]);
      toast({ title: 'Your account stays', tone: 'ok' });
    } finally {
      setBusy(false);
    }
  };
    ...
    <>
      ...
      <section className={styles.group} aria-labelledby="d-export">
        <div className={styles.groupHead}>
          <div>
            <h2 id="d-export" className={styles.groupTitle}>
              Download your data
            </h2>
            <p className={styles.groupLede}>
              One file with your profile, sign-in history, consent record, workspaces, every generation and the credit ledger. Media files are listed by name;
              download those from the Library.
            </p>
          </div>
          <Button variant="subtle" onClick={() => void exportAll()} loading={exporting}>
            Download JSON
          </Button>
        </div>
      </section>
      ...
      <section className={styles.group} aria-labelledby="d-keep">
        <div className={styles.groupHead}>
          <div>
            <h2 id="d-keep" className={styles.groupTitle}>
        ...
        <div style={{ display: 'grid', gap: 'var(--s-2)', fontSize: 'var(--t-2)', color: 'var(--ink-soft)', lineHeight: 'var(--lh-body)' }}>
          <p>
            Your uploads and the things made from them stay until you delete them or your account. Provider copies of a source photo are deleted after the
            generation finishes.
          </p>
          <p>
            The credit ledger is kept for seven years after the account closes, because it is a financial record. It is anonymised — no name, no email — the day
      ...
      {!profile.deletion && (
        <section className={styles.group} data-danger="true" aria-labelledby="d-del">
          <div className={styles.groupHead}>
            <div>
              <h2 id="d-del" className={styles.groupTitle}>
                Delete your account
              </h2>
              <p className={styles.groupLede}>
                Thirty days after you ask, everything is gone: workspaces you own, their media, their credits. Signing in before then and pressing one button
                cancels it.
              </p>
            </div>
      ...
      <Dialog
        ...
        description="We wait thirty days, then delete everything. Unused credits are not refunded. If you own a workspace that other people use, hand it over first."
        footer={
          <>
            <Button variant="ghost" onClick={() => setOpen(false)}>
              Keep it
            </Button>
            <Button variant="danger" onClick={() => void requestDeletion()} loading={busy} disabled={typed !== 'DELETE'}>
              Schedule deletion
            </Button>
          </>
        }
