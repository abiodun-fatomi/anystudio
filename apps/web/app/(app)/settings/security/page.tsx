...
export default function SecurityPage() {
  ...
  const pages = useCursorPages<ActivityRow>(
    async (cursor, take) => {
      const a = await api.account.activity({ take, cursor: cursor ?? undefined });
      return { rows: a.rows, nextCursor: a.nextCursor };
    },
    { size: 25 },
  );
  const activity = pages.rows;
  const loadLists = useCallback(async () => {
    await Promise.all([loadSessions(), pages.reset()]);
  }, [loadSessions, pages.reset]);
  useEffect(() => {
    void loadSessions();
  }, [loadSessions]);
  ...
  return (
    <>
      ...
      <section className={styles.group} aria-labelledby="s-act">
        ...
        <div className={styles.rows} aria-busy={pages.busy || undefined}>
          {activity === null ? (
            <Skeleton style={{ height: 56 }} />
          ) : activity.length === 0 ? (
