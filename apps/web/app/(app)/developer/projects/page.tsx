...
export default function ProjectsPage() {
  ...
  const load = useCallback(async () => {
    try {
      setProjects(await api.developer.projects(workspace.id));
    } catch {
      setProjects([]);
    }
  }, [workspace.id]);
  ...
  const archive = async (p: DevProject, archived: boolean) => {
    try {
      await api.developer.updateProject(workspace.id, p.id, { archived });
      await load();
      toast({ title: archived ? 'Archived — its keys stop working' : 'Restored', tone: 'ok' });
    } catch (e) {
        ...
        <div className={styles.groupHead}>
          <div>
            ...
            <div className={styles.groupLede}>
              Keys and usage are grouped by project. One per storefront, one per environment — whatever you want to see separately.
            </div>
          </div>
          {isAdmin && (
            <Button
                ...
                <div className={styles.rowMain}>
                  <div className={styles.rowTitle}>
                    ...
                    {p.archivedAt ? (
                      ...
                      <Badge tone="ok">
                        {p.activeKeys ?? 0} active {p.activeKeys === 1 ? 'key' : 'keys'}
                      </Badge>
                    )}
                  </div>
                  <div className={styles.rowSub}>
                    <span style={{ fontFamily: 'var(--f-mono)' }}>{p.slug}</span>
                    {p.description ? ` · ${p.description}` : ''} · created {new Date(p.createdAt).toLocaleDateString()}
                  </div>
                </div>
                {isAdmin && (
                  <div className={styles.rowEnd}>
                    <Button variant="ghost" size="sm" href={`/developer/keys?project=${p.id}`}>
                      Keys
                    </Button>
                    <Button
                      variant="ghost"
                    ...
                    <Button variant="ghost" size="sm" onClick={() => archive(p, !p.archivedAt)}>
