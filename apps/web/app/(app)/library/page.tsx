...
function Library() {
  ...
  const doRemove = async () => {
    ...
    setBusy(true);
    try {
      await api.library.remove(workspace.id, remove.id);
      setItems((cur) => cur?.filter((i) => i.id !== remove.id) ?? cur);
      setOpen(null);
      setRemove(null);
      toast({ title: 'Removed from the library', tone: 'ok' });
    } catch (e) {
  ...
  const openItem = async (item: LibraryItem) => {
    setOpen(item);
    try {
      setOpen(await api.library.get(workspace.id, item.id));
    } catch {
      /* the list's copy still shows */
    }
  };
  const again = (item: LibraryItem) => {
    try {
      sessionStorage.setItem('anystudio:prefill', JSON.stringify({ toolId: toolFor(item.capability), params: item.params ?? {} }));
    } catch {
      /* fine */
    }
  ...
  return (
    <div className="rise">
      <PageHeader
        title="Library"
        lede="Everything you have made, searchable, in every size it was exported in."
        actions={
          <Button href="/studio" leading={<Icon.plus />}>
            Make something
          </Button>
        }
      />
      ...
      <div className={styles.bar}>
        <div className={styles.search}>
          <Input
            placeholder="Search by product, words in the prompt, or the copy itself"
            value={q}
        ...
        <div className={styles.chips} role="group" aria-label="Type">
          {TYPES.map((t) => (
            <button
              ...
              aria-pressed={type === t.id && view === 'items'}
              onClick={() => {
                setType(t.id);
                setView('items');
              }}
          ...
          <button type="button" className={styles.chip} aria-pressed={starred} onClick={() => setStarred((s) => !s)}>
            ★ Starred
          </button>
          <button
            type="button"
            ...
            onClick={() => setView((v) => (v === 'products' ? 'items' : 'products'))}
          >
            Products
          </button>
        </div>
      ...
      {product && (
        <div className={styles.crumb}>
          Showing <strong>{productTitle}</strong>{' '}
      ...
      {view === 'products' ? (
        products === null ? (
          ...
          <EmptyState
            ...
            body="Give a product a name when you make something and it shows up here with everything made for it."
          />
        ) : (
          <div className={styles.grid}>
            {products.map((p) => (
              ...
              >
                ...
                <span className={styles.cardBody}>
                  <span className={styles.cardTitle}>{p.title ?? p.productKey}</span>
                  <span className={styles.cardMeta}>
                    {p.count} item{p.count === 1 ? '' : 's'} · last {when(p.lastAt)}
                  </span>
                </span>
              </button>
            ))}
              ...
              <div key={item.id} className={styles.card} data-type={item.type}>
                <button type="button" className={styles.thumb} onClick={() => void openItem(item)} aria-label={`Open ${item.title ?? TYPE_WORD[item.type]}`}>
                  {item.thumbUrl ? (
                    ...
                    <span className={styles.copyThumb}>{excerpt(item.text)}</span>
                  ) : item.type === 'audio' ? (
                    <span className={styles.audioThumb}>
                      <Icon.music width={28} height={28} />
                      <span>{excerpt(item.text).slice(0, 80)}</span>
                    </span>
                  ) : (
                    <Icon.library />
                  )}
                ...
                <div className={styles.cardBody}>
                  <div className={styles.cardTitle}>
                    <span>{item.title ?? TYPE_WORD[item.type]}</span>
                    <button
                      type="button"
                      className={styles.star}
                      aria-pressed={item.favourite}
                      aria-label={item.favourite ? 'Unstar' : 'Star'}
                      onClick={() => void patch(item, { favourite: !item.favourite })}
                    >
                      {item.favourite ? '★' : '☆'}
                    </button>
                  </div>
                  <div className={styles.cardMeta}>
                    <Badge mono>{TYPE_WORD[item.type]}</Badge> {when(item.createdAt)} · {item.credits} cr
                  </div>
                </div>
              </div>
            ))}
      ...
      <Dialog
        ...
        title={open ? <TitleEditor item={open} onSave={(t) => void patch(open, { title: t })} /> : ''}
        wide
        footer={
          open ? (
            <>
              <Button variant="ghost" onClick={() => setRemove(open)}>
                Remove
              </Button>
              <span style={{ flex: 1 }} />
              {open.type === 'image' && open.outputs.find((o) => o.role === 'image') && (
                <Button variant="subtle" onClick={() => router.push(`/studio?source=${encodeURIComponent(open.outputs.find((o) => o.role === 'image')!.key)}`)}>
                  Use as source
                </Button>
              )}
              {open.type === 'image' && open.outputs.find((o) => o.role === 'image') && (
                <Button
                  variant="subtle"
                  onClick={() => router.push(`/studio?tool=video&source=${encodeURIComponent(open.outputs.find((o) => o.role === 'image')!.key)}`)}
                >
                  Make a video
                </Button>
              )}
      ...
      <ConfirmDialog
        ...
        description="It disappears from every list. The credits it cost are not refunded — it was made. Support can bring it back for a while if you change your mind."
        confirmLabel="Remove"
        danger
      />
    </div>
      ...
      <div className={styles.preview}>
        {video ? (
          ...
          <div className={styles.audioBox}>
            ...
            {lockedTrack && (
              <span className={styles.cardMeta} style={{ whiteSpace: 'normal' }}>
                This is the 30-second preview. Unlock the full song from the studio result, or make it again.
              </span>
            )}
          </div>
        ) : item.text ? (
          ...
          <span className={styles.cardMeta}>No preview</span>
        )}
      </div>
      <div className={styles.side}>
        <div className={styles.meta}>
          <span>
            <Badge mono>{TYPE_WORD[item.type]}</Badge>
          </span>
        ...
        {files.length > 0 && (
          <div className={styles.files}>
            <div className={styles.filesHead}>
              <span>Files</span>
              <a className={styles.dl} href={api.library.downloadUrl(workspaceId, item.id)}>
                Download all (.zip)
              </a>
            </div>
            {files.map((o) => (
              <a key={o.key} className={styles.file} href={o.url ?? '#'} download target="_blank" rel="noreferrer" aria-disabled={!o.url}>
                ...
                <span className={styles.cardMeta}>
                  ...
                  {o.durationMs ? `${Math.round(o.durationMs / 1000)}s` : ''} {o.bytes ? `· ${(o.bytes / 1_000_000).toFixed(1)} MB` : ''}
                </span>
              </a>
            ))}
          </div>
