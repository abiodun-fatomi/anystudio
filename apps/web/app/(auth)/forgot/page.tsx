...
export default function ForgotPage() {
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      setError('We could not send that just now. Try again in a minute.');
    } finally {
      setBusy(false);
    }
  }
    ...
    return (
      <div>
        ...
        <p style={{ color: 'var(--muted)', marginTop: 10, lineHeight: 1.55 }}>
          If <strong>{email}</strong> has an account, a reset link is on its way. It works for 30 minutes. No email? Check spam, then{' '}
          <button
            type="button"
  ...
  return (
    <form onSubmit={submit} noValidate>
      ...
      <div className="field" style={{ marginTop: 26 }}>
        ...
        <input id="email" className="inp" type="email" autoComplete="email" autoFocus value={email} onChange={(e) => setEmail(e.target.value)} required />
      </div>
      {error && (
      ...
      <p style={{ marginTop: 22, fontSize: 14, textAlign: 'center' }}>
        <Link href="/login">Back to sign in</Link>
      </p>
    </form>
  );
