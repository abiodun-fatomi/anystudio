...
function EmailChange() {
  ...
  useEffect(() => {
    ...
    let live = true;
    api.account.confirmEmailChange(token).then((r) => { if (live) setState(r.status === 'changed' ? 'done' : 'invalid'); }).catch(() => { if (live) setState('invalid'); });
    return () => { live = false; };
  }, [params]);

  if (state === 'working') return <p style={{ color: 'var(--muted)' }} aria-live="polite">Switching your email…</p>;
  if (state === 'done') {
    return (
      <div>
  ...
  return (
    <div>
      ...
      <p style={{ color: 'var(--muted)', marginTop: 10, lineHeight: 1.55 }}>Email-change links work once, for 24 hours, and only if nobody has taken that address since. Start again from Settings.</p>
      <Link href="/settings/profile" className="btn" style={{ marginTop: 22 }}>Open Settings</Link>
    </div>
  );
}

export default function Page() { return <Suspense fallback={null}><EmailChange /></Suspense>; }
