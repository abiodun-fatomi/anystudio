...
export interface UploadProgress { loaded: number; total: number; pct: number }
...
export async function uploadFile(
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    xhr.upload.onprogress = (e) => { if (e.lengthComputable) onProgress?.({ loaded: e.loaded, total: e.total, pct: Math.round((e.loaded / e.total) * 100) }); };
...
function guessMime(name: string): string {
  ...
  return ({ jpg: 'image/jpeg', jpeg: 'image/jpeg', png: 'image/png', webp: 'image/webp', heic: 'image/heic', gif: 'image/gif', mp4: 'video/mp4', mov: 'video/quicktime', mp3: 'audio/mpeg', m4a: 'audio/mp4', wav: 'audio/wav' } as Record<string, string>)[ext] ?? 'application/octet-stream';
}
