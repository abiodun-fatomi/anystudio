...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const toggleRail = () =>
    setRail((r) => {
      const next = r === 'full' ? 'icons' : 'full';
      try {
        localStorage.setItem('anystudio:rail', next);
      } catch {
        /* fine */
      }
      return next;
  ...
  const setWidth = (w: number | null) => {
    setRailWidth(w);
    try {
      if (w === null) localStorage.removeItem('anystudio:rail-width');
      else localStorage.setItem('anystudio:rail-width', String(w));
    } catch {
      /* fine */
    }
  };
  const startDrag = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    const startW = railWidth ?? RAIL_DEFAULT;
    setDragging(true);
    ...
    const up = (ev: PointerEvent) => {
      ...
      setDragging(false);
      setWidth(Math.min(RAIL_MAX, Math.max(RAIL_MIN, startW + ev.clientX - startX)));
    };
    ...
    >
      ...
      <aside className={styles.rail} aria-label="Primary">
        ...
        <div className={styles.railFoot}>
          {me.canSwitchToStaff && (
            <a href={siblingOrigin(window.location.host, 'admin')} className={styles.staffLink} style={{ fontFamily: 'var(--f-mono)' }}>
              Staff console →
            </a>
          )}
          <button
            ...
            onClick={toggleRail}
            aria-label={rail === 'full' ? 'Collapse the sidebar' : 'Expand the sidebar'}
            aria-expanded={rail === 'full'}
            title={rail === 'full' ? 'Collapse' : 'Expand'}
          >
        ...
        <div
          ...
          aria-label="Resize the sidebar (drag, or arrow keys; Home resets)"
            ...
            {(close) => (
              <>
                <div className={styles.menuUser}>
                  <span className={styles.menuName}>{me.user.name ?? 'Your account'}</span>
                  <span className={styles.menuEmail}>{me.user.email ?? me.user.phone}</span>
                </div>
                <MenuSeparator />
                <MenuItem href="/settings/profile" onSelect={close} leading={<Icon.user />}>
                  Profile
                </MenuItem>
                <MenuItem href="/settings" onSelect={close} leading={<Icon.settings />}>
                  Settings
                </MenuItem>
                <MenuSeparator />
                <div className={styles.themeRow} role="group" aria-label="Theme">
                  ...
                  <div className={styles.themeSeg} role="radiogroup" aria-label="Theme">
                    <button type="button" role="radio" aria-checked={theme === 'light'} aria-label="Light" onClick={() => pickTheme('light')}>
                      <Icon.sun width={16} height={16} />
                    </button>
                    <button type="button" role="radio" aria-checked={theme === 'system'} aria-label="System" onClick={() => pickTheme('system')}>
                      Auto
                    </button>
                    <button type="button" role="radio" aria-checked={theme === 'dark'} aria-label="Dark" onClick={() => pickTheme('dark')}>
                      <Icon.moon width={16} height={16} />
                    </button>
                  </div>
                </div>
                <MenuSeparator />
                <MenuItem onSelect={() => void signOut()} leading={<Icon.logout />}>
                  Sign out
                </MenuItem>
              </>
            )}
