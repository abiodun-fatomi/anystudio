...
export function WorkspaceSwitcher() {
  ...
  const create = async () => {
    ...
    try {
      ...
      toast({ title: `${ws.name} created`, body: type === 'ORGANIZATION' ? 'You are in it now. The Developer section is yours.' : 'You are in it now.', tone: 'ok' });
  ...
  const trigger = (
    <button type="button" className={styles.wsBtn} data-tour="workspace" aria-label={`Workspace: ${workspace.name}. Switch or create a workspace`}>
      ...
      <span className={styles.wsText}><span className={styles.wsName}>{workspace.name}</span><span className={styles.wsType}>{WS_TYPE[workspace.type] ?? workspace.type}</span></span>
      ...
      <Popover menu trigger={trigger}>
        {(close) => (
          <>
            ...
            {workspaces.map((w) => (
              <MenuItem key={w.id} onSelect={() => { if (w.id !== workspace.id) switchWorkspace(w.id); close(); }}
                leading={w.id === workspace.id ? <Icon.check width={16} height={16} /> : <span style={{ width: 16, display: 'inline-block' }} />}>
                <span style={{ display: 'grid', lineHeight: 1.2 }}><span>{w.name}</span><span className={styles.wsType}>{WS_TYPE[w.type] ?? w.type}</span></span>
              </MenuItem>
            ))}
            <MenuSeparator />
            <MenuItem href="/settings/workspace" onSelect={close} leading={<Icon.settings width={16} height={16} />}>Workspace settings</MenuItem>
            <MenuItem onSelect={() => { close(); setOpen(true); }} leading={<Icon.plus width={16} height={16} />}>New workspace…</MenuItem>
          </>
        )}
      </Popover>
      <Dialog open={open} onClose={() => !busy && setOpen(false)} title="New workspace"
        description="A workspace has its own credits, brand kit, library and members. You own it; invite people from Settings → Workspace." locked={busy}
        ...
        <div style={{ display: 'grid', gap: 'var(--s-4)' }}>
          <SegmentedControl<NewType> label="Kind of workspace" value={type} onChange={setType}
            items={[{ id: 'BUSINESS', label: 'Business' }, { id: 'ORGANIZATION', label: 'Organization' }]} />
          ...
          <Input label="Workspace name" value={name} onChange={(e) => setName(e.target.value)} placeholder={type === 'BUSINESS' ? 'Ada Fabrics' : 'Acme Commerce'} maxLength={80} autoFocus />
        </div>
      </Dialog>
    </>
  );
