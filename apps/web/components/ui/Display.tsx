...
export function Badge({
  ...
  return (
    <span className={cx(styles.badge, mono && styles.mono, className)} data-tone={tone}>
...
export function Card({ pad = true, interactive, className, children, ...rest }: HTMLAttributes<HTMLDivElement> & { pad?: boolean; interactive?: boolean }) {
  return (
    <div className={cx(styles.card, pad && styles.pad, interactive && styles.interactive, className)} tabIndex={interactive ? 0 : undefined} {...rest}>
      {children}
    </div>
  );
}
...
export function Progress({ value, label, detail, className }: { value: number | null; label?: ReactNode; detail?: ReactNode; className?: string }) {
  ...
  return (
    <div
      className={cx(styles.prog, className)}
      role="progressbar"
      aria-valuemin={0}
      aria-valuemax={100}
    ...
    >
      {(label || detail) && (
        <div className={styles.progHead}>
          <span>{label}</span>
          <span className={styles.progVal}>{detail ?? (pct !== null ? `${Math.round(pct)}%` : '')}</span>
        </div>
      )}
      <div className={styles.track}>
        <div className={styles.fill} style={{ width: pct === null ? undefined : `${pct}%` }} data-indeterminate={pct === null || undefined} />
      </div>
    </div>
  );
}
...
export function Stat({ label, value, sub, className }: { label: ReactNode; value: ReactNode; sub?: ReactNode; className?: string }) {
  return (
    <div className={cx(styles.stat, className)}>
      ...
      {sub && <div className={styles.statS}>{sub}</div>}
    </div>
  );
}
  ...
  return (
    <div className={styles.empty} role="alert">
      ...
      <div className={styles.emptyBody}>
        {message ?? 'The server did not answer as expected.'} If this keeps happening, the service may be mid-deploy — try again in a minute.
      </div>
      {onRetry && (
