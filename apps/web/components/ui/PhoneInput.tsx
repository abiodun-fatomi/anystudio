...
export function PhoneInput({
  ...
  useEffect(() => {
    setMounted(true);
    if (!value.country) onChange(compose(guessCountry(), ''));
  }, []);
  const dial = value.country ? getCountryCallingCode(value.country) : '';
  const options = useMemo(
    () => (mounted ? [...PINNED.map((cc) => COUNTRIES.find((c) => c.cc === cc)!).filter(Boolean), ...COUNTRIES.filter((c) => !PINNED.includes(c.cc))] : []),
    [mounted],
  );
  const showError =
    error ??
    (touched && value.national && !value.valid ? `That does not look like a ${COUNTRIES.find((c) => c.cc === value.country)?.name ?? ''} number.` : null);
      ...
      <div className={styles.row}>
        <div className={styles.country}>
          <span className={styles.flag} aria-hidden="true">
            {mounted && value.country ? flag(value.country) : '🌍'}
          </span>
          <span className={styles.dial}>{mounted ? `+${dial}` : '+'}</span>
          <select
            aria-label="Country"
            value={value.country}
            onChange={(e) => onChange(compose(e.target.value as CountryCode, value.national))}
            className={styles.select}
          >
            {options.map((c, i) => (
              <option key={c.cc} value={c.cc}>
                {c.name} (+{c.dial}){i === PINNED.length - 1 ? ' ──' : ''}
              </option>
            ))}
          </select>
        </div>
        <input
          id={fieldId}
          className={`inp ${styles.number}`}
          type="tel"
          inputMode="tel"
          autoComplete="tel-national"
          placeholder={placeholderFor(value.country)}
          value={value.national}
          required={required}
          autoFocus={autoFocus}
          aria-invalid={showError ? true : undefined}
          aria-describedby={showError ? `${fieldId}-err` : hint ? `${fieldId}-hint` : undefined}
          onChange={(e) => onChange(compose(value.country, e.target.value))}
          onBlur={() => setTouched(true)}
          onPaste={(e) => {
            const text = e.clipboardData.getData('text');
            if (/^\+/.test(text.trim())) {
              e.preventDefault();
              onChange(fromE164(text.trim(), value.country));
            }
          }}
        />
      </div>
      {showError ? (
        <p id={`${fieldId}-err`} className="err" role="alert" style={{ marginTop: 6 }}>
          {showError}
        </p>
      ) : hint ? (
        <p id={`${fieldId}-hint`} style={{ marginTop: 6, fontSize: 13, color: 'var(--muted)' }}>
          {hint}
        </p>
      ) : null}
    </div>
...
export function compose(country: CountryCode, typed: string): PhoneValue {
  ...
  try {
    e164 = typed.trim() ? parsePhoneNumber(typed, country).number : null;
  } catch {
    e164 = null;
  }
