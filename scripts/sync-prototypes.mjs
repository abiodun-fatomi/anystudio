...
const PAGES = [
  ...
  {
    path: '/pricing', name: 'pricing', symbol: 'PRICING', body: promote(promoted.get('pricing')), keepAnchors: false,
  ...
  {
    path: '/developers', name: 'developers', symbol: 'DEVELOPERS', body: promote(promoted.get('api')), keepAnchors: false,
...
for (const page of PAGES) {
  const doc = [headFor(head, page), markCurrent(resolveAnchors(nav, page.keepAnchors), page.path),
    page.body, resolveAnchors(footer, page.keepAnchors), script].join('\n');
